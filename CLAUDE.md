...
- **A `Str` primitive has exactly one implementation, shared by the `Str` method, the `nqp::` op, the VM opcode and the TRIR op** ([ADR-0117](docs/adr/0117-str-methods-and-nqp-ops-share-one-routine.md), user decision 2026-09-23). Rakudo's `.substr`/`.index(:i)`/`.starts-with`/`.flip`/`x` *are* `nqp::substr`/`indexic`/`eqat`/`flip`/`x`; mutsu kept three drifting copies (grapheme-indexed methods, codepoint-indexed nqp ops on a private `Vec<char>` memo, codepoint-indexed TRIR ops on another). The single home is [`src/builtins/str_prim/`](src/builtins/str_prim/mod.rs): positions are graphemes (MoarVM strings are NFG), `:i`/`:m` go through its `Fold`, and callers differ only in how they report an edge case (`Nil`/`Failure` vs. `-1` vs. a native int). When you add or fix a string method or op, put the logic there and call it from every layer — never a private search, fold, slice or char cache. `make check-str-prims` (a `make test` prerequisite and a CI step) fails on a hand-rolled string primitive in the nqp/TRIR/VM-nqp layers, and `t/vm/nqp-str-prim-parity.t` pins that each op agrees with its method.

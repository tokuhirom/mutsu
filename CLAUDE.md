...
- **Level-1 (cycle collector on `Arc`) is the accepted approach** (ADR-0001 §3-8): non-moving + refcount also makes JIT far easier to land — no GC stack maps, no forwarding, no write barriers; JIT just emits `Arc` inc/dec. With the scalar/container type filter, JIT-native int loops pay zero GC/refcount cost, so hot paths reach near level-2 speed anyway. **Order: 3a (GC) → 3b (NaN-boxing, now a JIT enabler) → 4 (JIT).** Level-2 (full VM redesign for MoarVM-style precise tracing) is a long-term option only if level-1 JIT hits a *measured* refcount ceiling — do NOT start it without a new/updated ADR.

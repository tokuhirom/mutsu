...
- **When you discover a bug or missing feature that is too large to fix in the current or the next session, file it as a GitHub issue on `tokuhirom/mutsu` — do NOT append it to PLAN.md.** (This is a deliberate, incremental switch away from the PLAN.md-append style: many small PRs editing the same PLAN.md lines conflict constantly, whereas a fresh issue conflicts with nothing — exactly the reasoning behind the per-entry `news/` files.) Do not let a hard, high-blast-radius finding evaporate at the end of a session — capture the same detail a PLAN.md entry would (root cause, affected files, why it is large, any repro) in the issue body so a future session can pick it up, and apply the `bug` or `enhancement` label. Filing an issue needs no branch and no PR.
  - **NEVER create an issue (or PR) against any repository other than `tokuhirom/mutsu`. This is absolute, and it is not hypothetical: an AI has actually mis-filed a mutsu issue against a Raku-org repo before.** That is precisely why this rule and the Raku-org ban above exist. Always pass `--repo tokuhirom/mutsu` explicitly to `gh issue create` so a stray upstream remote (the vendored `roast`/`raku-doc` trees, an added upstream, or a wrong working directory) can never redirect it elsewhere; check the `--repo` value before you hit enter. (`gh issue create` occasionally errors on this repo with a Projects-classic GraphQL warning after the issue is already created — verify with `gh issue list --repo tokuhirom/mutsu` before retrying, to avoid filing a duplicate.)
  - PLAN.md stays the home for *planned* strategic/campaign work and current-quarter priorities — not ad-hoc discovered-bug records. Existing PLAN.md bug entries can remain where they are; migrate them to issues opportunistically when you next touch them (incremental, not a big-bang move).

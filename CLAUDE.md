...
- `roast/S04-exceptions/exceptions-alternatives.t` — occasional per-test timeout (exit 124, `Failed: 0`, "planned 3 ran 2") on the **jit-stress** CI job; seen twice on 2026-07-15 on docs-only PRs (#4548, #4551), so it cannot be a code regression. **Root cause is a known perf pathology, not load noise**: subtest 3 parses the subprocess's JSON stderr with `JSON::Tiny::Grammar`, and that parse takes **~12.6s** (raku: 2ms) due to sep-quantifier backtracking in `rule pairlist { <pair> * % \, }` cloning `RegexCaptures` per backtrack — see the regex item in PLAN.md §5 (measured 2026-07-15). It fits the 30s per-test budget only with a slim margin, so parallel CI load plus JIT-compile overhead occasionally pushes it over. Did not reproduce locally (release, `MUTSU_JIT=on`): 5× serial + 12× 4-way-parallel all PASS. The real fix is the PLAN §5 `RegexCaptures.clone()` reduction; until then a timeout here is flaky, but a concrete `not ok` is real.

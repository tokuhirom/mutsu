...
- **Providing an ecosystem module "natively" (BATTERIES.md rung 3) is BANNED going forward (user decision, 2026-08-01).** When a module needs deep machinery (Metamodel, EXPORTHOW, slangs, NQP guts), the answer is rung 2: grow the interpreter until the real, vendored upstream module runs verbatim. "The implementation is large" is not a reason to fall back to a native reimplementation — that is exactly the private-dialect risk BATTERIES.md §1 warns about. If you believe native provision is genuinely unavoidable, STOP and ask the user; never ship it unilaterally. Existing native providers (Test, JSON::Fast, NativeCall) predate this rule; retire one the way `Pod::To::Text` was retired — it was a native `pod2text` builtin until the real rakudo core module was vendored to `modules/Rakudo-Core/` and three general interpreter bugs were fixed to run it (`docs/batteries/pod-to-text.md`) — or the way the native `monitor` declarator stopgap (#5640) was retired: the EXPORTHOW::DECLARE campaign grew the MOP until OO::Monitors ran verbatim as a bundled battery (`news/2026-08/exporthow-declare-mop.md`). But **measure before assuming a provider is retirable**: `NativeCall` was measured on 2026-08-01 and is not (it needs `use QAST:from<NQP>`, MoarVM dispatch programs, and 61 missing `nqp::` ops; see `todo/deep/nativecall-cannot-be-vendored.md`), so it stays a justified rung-3 provider.
...
- **npm bootstrap (one-time):** npm only allows trusted publishers to be configured for an existing package. Publish the first `mutsu` version interactively with 2FA, then configure its GitHub Actions trusted publisher as user `tokuhirom`, repository `mutsu`, workflow `release.yml`, allowed action `npm publish`. Every later tag publishes without a token.
...
- `roast/S02-types/baghash.t` tests 121/122/261/262 (`.roll(100) (1)/(2)`) — **statistically flaky by design**, not a mutsu bug: the assertions bound a Binomial(100, 1/3) sample (`2 < a < 75`, `a + 2 < b`), which fails with small probability even with a correct RNG. Verified 2026-07-15: the weighted-roll distribution is correct (mean a = 32.6 over 30 fresh processes, 32.9 over 200 in-process runs of `roll: 100`; exactly 1/3 at n=10000). A failure ONLY in these subtests passes on retry; any other subtest failing is real.
- `roast/S02-types/bag.t` tests 114-117 (`.roll(100) (1)/(2)`, twice) — the plain-`Bag` member of the same family: `Bag.new("a","b","b").roll` bounded by the same Binomial(100, 1/3) assertions, the second of which (`a + 2 < b`) fails whenever `a >= 49`. Verified 2026-07-25 (CI run 30134566008 failed test 117 only under gc-stress; 5×/5× green locally; mean a = 33.44 over 2000 samples against an expected 33.33, with the test-117 bound violated 2/2000). Same rule: a failure ONLY in these subtests passes on retry; any other subtest failing is real.
- `roast/S02-types/mixhash.t` tests 128/129/130/131 (`.roll(100) (1)/(2)`, `.roll(*)[^100] (1)/(2)`) — the MixHash twin of the baghash class above: the same Binomial(100, 1/3) bounds (`2 < a < 75`, `a + 2 < b`) on a `(a => 1, b => 2).MixHash` roll. Verified 2026-07-16 (CI run 29462324997 failed test 129 only; locally 5×/5× green under the gc-stress env and the distribution is exactly 1/3 at n=10000). Same rule: a failure ONLY in these subtests passes on retry; any other subtest failing is real. (Tests 179/180 are `# TODO in flux` expected failures — a local `grep -c "not ok"` counts them; check the exit status / `Failed:` count instead.)
...
- **When you discover a bug or missing feature that is too large to fix in the current or the next session, record it as a new file under `todo/` — one file per finding — NOT by appending to PLAN.md.** Split by the nature of the work: `todo/tickets/<slug>.md` for small, self-contained, no-design-needed slices; `todo/deep/<slug>.md` for deep, high-blast-radius problems that need design/an ADR; and `todo/perf/<slug>.md` when mutsu is *correct but slow* and the next step is profiling (see `todo/README.md`). A perf-flavoured finding that also fixes a genuinely wrong answer stays in `tickets/`/`deep/` — do not bury a real bug behind a benchmark. Like the per-entry `news/` files, a brand-new file conflicts with nothing on merge, which is the whole point: PLAN.md appends collided constantly across parallel small PRs. Give the file an H1 title and a prose body (no frontmatter) with root cause, affected files, why it is large, and a minimal repro. Create the subdirectory lazily — just `Write` the file. Do not let a hard finding evaporate at the end of a session; the file is worth a docs-only PR.
  - **When the finding is resolved, `git mv` it to `news/YYYY-MM/<slug>.md`** and rewrite it as an accomplishment; if it evaporated, delete it. `todo/` holds only *open* findings.
  - **Do NOT file these as GitHub issues.** Bug records stay in-repo as files. Do not open GitHub issues on your own initiative, and never against any repository other than `tokuhirom/mutsu` (an AI has actually mis-filed a mutsu issue against a Raku-org repo before).
...
- **Task selection order:**
  ...
  4. `todo/deep/` and `todo/tickets/`, oldest-first (see "The todo/deep + todo/tickets parallel-agent pipeline" below) — both queues get worked in parallel, not one after the other, so neither backlog grows unchecked while the other drains. **`todo/perf/` is NOT part of that pipeline**: perf work is batched into its own profiling-heavy session and its implementation agent runs solo (see below).

### The todo/deep + todo/tickets parallel-agent pipeline

When working through the `todo/` backlog (as opposed to a single focused task), run it as a batch pipeline of disposable, fully-independent worktree agents rather than one long serial session:

- **One fresh agent per ticket, never reused.** Every `todo/deep/<slug>.md` or `todo/tickets/<slug>.md` gets its own `Agent` call with `isolation: "worktree"` and a fully self-contained prompt (the ticket's own content summarized + exact task steps + the PR workflow instructions) — a subagent starts with zero context, so the prompt must stand alone. Once an agent's PR is merged, it is done; do not send it a new, unrelated ticket. The next ticket always gets a brand-new `Agent` call, not a `SendMessage` to a "graduated" one.
- **Oldest-first, `todo/deep/` and `todo/tickets/` interleaved.** Pick a small batch (3, per the build-agent cap above) from `ls -tr todo/deep/*.md todo/tickets/*.md | head` each round (**not** `todo/perf/` — see the perf rule below), skim each file first to write an informed prompt (don't dispatch blind) — a ticket's stated blast radius/priority section often changes what the right prompt asks for (e.g. "check whether this is worth it before implementing" needs a corpus-grep triage step, not a straight implementation).
...
- **A ticket does not always end in a code fix.** A large fraction of investigations correctly conclude one of: the ticket is stale (already fixed by something else) — verify, close it out to `news/`, and pin a regression test so it can't silently regress; the ticket needs a priority/scope judgment call before any code (e.g. "is deep MOP machinery worth it for one dist?") — grep the available corpus for other consumers of the same gap and let that evidence decide implement-vs-defer; or the real fix is genuinely bigger than the ticket assumed — split off a narrower, better-scoped ticket (or ADR) recording exactly what was learned, rather than forcing an undersized fix through. All of these are legitimate, valuable outcomes — do not treat "closed the ticket without shipping code" as a failure.
...
- **`todo/perf/` is worked separately, and its implementation agent runs SOLO.** Never put a perf finding into the parallel batch above: two perf agents on one box produce measurements that drift and never converge, and the whole point of a perf finding is a trustworthy number. Batch several `todo/perf/` files into one profiling-heavy session instead, so the profiler setup is amortized. Numbers that end up in a document must come from the bench CI (see "Benchmark numbers in documents come from the bench CI" above), not from that session's local runs.

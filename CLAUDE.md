...
- **When you discover a bug or missing feature that is too large to fix in the current or the next session, record it as a new file under `todo/` — one file per finding — NOT by appending to PLAN.md.** Split by the nature of the work: `todo/tickets/<slug>.md` for small, self-contained, no-design-needed slices, and `todo/deep/<slug>.md` for deep, high-blast-radius problems that need design/an ADR (see `todo/README.md`). Like the per-entry `news/` files, a brand-new file conflicts with nothing on merge, which is the whole point: PLAN.md appends collided constantly across parallel small PRs. Give the file an H1 title and a prose body (no frontmatter) with root cause, affected files, why it is large, and a minimal repro. Create the subdirectory lazily — just `Write` the file. Do not let a hard finding evaporate at the end of a session; the file is worth a docs-only PR.
...
- Keep at most 4 concurrent agents (not more, to avoid quota exhaustion) — but this caps agents actually *doing work*, not agents idling on a CI run: an agent whose PR is open and waiting on GitHub Actions uses no local CPU, so it is fine to launch a fresh agent past the nominal cap while others are purely CI-pending (user-confirmed 2026-08-20). **Even so, never exceed 10 agents in total (working + CI-idle combined)** (user-confirmed 2026-08-20) — check `ListAgents` before launching a new one when the count is already high. Clean up worktrees per the "Worktree cleanup" section.
- **Task selection order:**
  ...
  4. `todo/deep/` and `todo/tickets/`, oldest-first (see "The todo/deep + todo/tickets parallel-agent pipeline" below) — both queues get worked in parallel, not one after the other, so neither backlog grows unchecked while the other drains.

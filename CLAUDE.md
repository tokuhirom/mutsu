...
- **Every built-in method / routine / `nqp::` op implementation states its complexity in a `// Cost:` comment** (user decision, 2026-09-23). When you add a method arm, a native method, a runtime method handler, a builtin routine or an `nqp::` op — or change what an existing one costs — put one `// Cost: O(..), <var> = <meaning>.` line directly above it, in the format [docs/complexity-annotations.md](docs/complexity-annotations.md) defines. If its bound is worse than Rakudo's (`Rakudo: O(..)`) or MoarVM's (`MoarVM: O(..)`), add that suffix **and file a `todo:perf` issue** for it (`-- see #NNNN` with the real number; never leave a literal `#NNNN`). A fix that removes a deficit drops the suffix in the same PR and re-runs the family's `scripts/*-complexity-check.sh` case. The `nqp::`, `Str` and `Array`/`List` families and every VM opcode arm (`exec_one_dispatch`) are fully annotated; a new `OpCode` arm needs its `// Cost:` line like any method; other families (Hash, numeric, IO, ...) are annotated as they are touched, and a whole-family audit follows the same recipe as those three (see the news entries they link).

...
- **アロケーション失敗 abort** 【唯一の uncatchable なプロセス abort】: catchable な Rust panic
  ...
  落とさない。残るのは **catch_unwind で捕捉できない** アロケーション失敗 abort: 配列 autoviv の
  巨大 index は `Vec` が実際に巨大確保を試み `handle_alloc_error`→`abort()` する
  (実機: `my @a; @a[9999999999999]=1` → `memory allocation of 480000000000000 bytes failed`、
  core dump)。これは main/worker を問わず発生し、**raku も同条件で `MoarVM panic` で落ちる**ため
  挙動としては raku 相当。**改善 (任意)**: autoviv index に上限を設け、確保を試みる前に catchable な
  `X::` を投げる (raku より親切になるが必須ではない)。
...
初版ロードマップの上位 (遅延リスト・並行ライブセル・メイン/worker panic 境界・regex 統合・
`.Supply` クラッシュ・`Value` 縮小) はすべて完了済み。残る課題:

| # | 項目 | 区分 | 効果 |
|---|------|------|------|
| 1 | 配列 autoviv index の上限ガード (確保前に catchable `X::`) / Supply worker panic の QUIT 伝播 | 堅牢性 (任意) | 低〜中 |

...
The pair value is smartmatched against the value in the Unicode Character
Database.
...
you refer to C<lol()>, you can get each individual sublist as a C<List>
...
and fail if the end of text is not reached.  (The C<TOP> rule can check against C<$> itself
...
The final position can be determined by examining the returned C<Match> object.
...
(The actual implementation of C<s///> must return a C<Match> to make
...
One other difference between the C<s///> and C<.subst> forms is that,
while C<.subst> returns the modified string (and cannot, therefore,
be used as a smart matcher), the C<s///> form always returns either a
C<Match> object to indicate to smartmatch that it was successful,
or a C<Nil> value to indicate that it was not.

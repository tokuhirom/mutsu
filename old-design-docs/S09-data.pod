...
independent lists, which can be read lazily independently of one other.
...
which depends on the context.  Such a semicolon list treats each of its
sublists separately, deferring the context of the sublist until
...
Any mention of an expression within a Capture (that is, an argument list)

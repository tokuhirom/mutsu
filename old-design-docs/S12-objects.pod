...
For normal user classes, C<P6opaque> is the default representation.
...
as C<@object>.  Each such return value is a C<List> or C<List> of C<List>
...
which means that at the end of the main compilation (C<LINK> time)
...
main C<LINK> time, at which point the compiler commits to its

...
Use of C<nextwith> allows the routine to be called without introducing
...
of that sigil, but sometimes you don't want that.  It is possible to bind
an argument to a simple name instead, which represents that argument in
its rawest form, with no commitment to structure or mutability.
...
Raw parameters make it relatively easy to program in a "sigilless" style,
if you desire:
...
raw value, whether it flattens or not depends on the context in which it is eventually used.
...
programming style known as SSA, Single Static Assignment (really binding),
which, among other benefits, gives the optimizer more guarantees about
which symbols can be considered "temporarily immutable".
...
than requesting individual elements as the flattening C<*> does.
...
=head2 One-argument slurpy binding

If you use a C<+> character to indicate a slurpy parameter, then it is
assumed that the rest of the positional arguments are intended to be iterated
with "single argument" semantics.  That is, we will take whatever is passed
in and iterate the top level of that.  If the top level is a comma list,
then it works just as a C<**> slurpy.  If the top level is a single argument,
then that will be iterated.
...
Basically, this slurpy doesn't care whether the list you hand it was made
by syntactic commas or by some other means.  The other slurpies treat
syntactic commas as dominant even when there aren't any!
...
that knows how to deal with a list of values as a slice, such as C<zip>:
...
Here C<$foo> is a list of three lists, so
...
simply mark the attributes you want to work that way with C<is raw>:
...
object that has not yet had a context imposed.
...
prefixing with a backslash rather than by using C<is raw> directly.
...
[DEPRECATED] Specifies that the parameter is to be coerced to the given type.
...
making these optimizations after the main program's C<LINK> time,
...
after C<LINK> time.  This is done by recasting to C<SoftRoutine>.
...
Hence, any routine marked as soft before C<LINK> time is exempt
...
performed before C<LINK> time.  The optimizer may then harden
...
Note that installing a wrapper before C<LINK> time is specifically
I<not> one of the ways to mark a routine as soft.  Such a routine
may still be hardened at C<LINK> time despite being wrapped during
compile time.
...
with the C<unit> declarator (and ending in semicolon) is allowed at the outermost file scope if it is the

//! Unicode property data tables for uniprop.
//! Extracted from uniprop.rs to keep individual files under 500 lines.

use std::sync::OnceLock;

/// Unicode Block lookup.
pub(super) fn unicode_block(ch: char) -> String {
    let cp = ch as u32;
    static BLOCKS: OnceLock<Vec<(u32, u32, &'static str)>> = OnceLock::new();
    let blocks = BLOCKS.get_or_init(|| {
        let mut v: Vec<(u32, u32, &str)> = vec![
    ...
    match blocks.binary_search_by(|&(start, end, _)| {
        if cp < start {
        ...
        Ok(idx) => blocks[idx].2.to_string(),
        Err(_) => "No_Block".to_string(),
    }
}

/// Jamo short name lookup.
pub(super) fn unicode_jamo_short_name(ch: char) -> String {
    let cp = ch as u32;
    match cp {
        0x1100 => "G",
        0x1101 => "GG",

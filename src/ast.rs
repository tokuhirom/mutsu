...
pub(crate) enum Expr {
    ...
    AnonSubParams {
        ...
        /// `anon Str sub { }`, one candidate of an anonymous `multi sub`),
        ...
        /// RakuAST converter reads this flag instead of guessing.
        ///
        /// It ALSO selects the compile path: `false` compiles a `Block` (Mu
        /// -typed params, not a `return` boundary, and a `Block` gist —
        /// `-> $a { ... }`), `true` compiles a `Sub`. So a new construction
        /// site must set it to what the source actually wrote; getting it
        /// wrong changes `.^name` and `return` semantics, not just rendering.
        /// (This doc used to say the flag had no effect on execution, and
        /// several `sub` sites were left `false` on that basis.)
        is_sub: bool,
    },
    CallOn {
        target: Box<Expr>,

...
impl Interpreter {
    ...
    pub(super) fn exec_get_local_op(
        ...
        if val.is_nil() {
            ...
            // (`clone_for_thread` seeds every env var it can see). A name this
            // frame re-declared is a fresh binding that shadows it, and its Nil is
            // a real Nil — not a stale snapshot to refresh from the shared store.
            // `set_shared_var_sym` already masks the WRITE side on exactly this
            // set; without the same gate here the read resurrects the foreign
            // value (`my $x := f()` yielding Nil would see the other scope's
            // `$x`). #4650; pin t/shared-var-nil-redeclared-mask.t.

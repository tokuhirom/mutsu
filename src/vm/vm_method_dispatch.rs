...
impl Interpreter {
    ...
    /// The monitor lock guarding a method call, when the receiver is a
    /// concrete instance of a `monitor` class (OO::Monitors, provided
    /// natively) — directly or via inheritance. Construction/clone plumbing
    /// is exempt, matching the upstream module's wrap exclusions.
    fn monitor_guard_for(
        &mut self,
        class_name: &str,
        method_name: &str,
        target: Option<&Value>,
    ) -> Option<std::sync::Arc<crate::runtime::native_methods::state_lock::LockRuntime>> {
        if !crate::runtime::native_methods::any_monitor_class() {
            return None;
        }
        if matches!(
            method_name,
            "new" | "bless" | "BUILDALL" | "POPULATE" | "clone" | "BUILD" | "TWEAK"
        ) {
            return None;
        }
        ...
        let is_monitor = crate::runtime::native_methods::is_monitor_class(class_name)
            || self
                .class_mro(class_name)
                .iter()
                .any(|c| crate::runtime::native_methods::is_monitor_class(&c.resolve()));
        if !is_monitor {
            return None;
        }
        crate::runtime::native_methods::monitor_lock_for_instance(id)
    }

    /// Serialize `f` on the instance's monitor lock when one applies (the
    /// lock is REENTRANT, so nested sibling-method calls do not deadlock).
    /// Uses the same critical-section bracketing as `Lock.protect` so shared
    /// state pulled/committed by other threads stays coherent.
    pub(super) fn with_monitor_lock<T>(
        &mut self,
        class_name: &str,
        method_name: &str,
        target: Option<&Value>,
        ...
        let Some(rt) = self.monitor_guard_for(class_name, method_name, target) else {
            return f(self);
        };
        let me = crate::runtime::native_methods::current_thread_id();
        crate::runtime::native_methods::acquire_lock(&rt, me)?;
        self.enter_critical_section();
        let result = f(self);
        self.leave_critical_section();
        let _ = crate::runtime::native_methods::release_lock(&rt, me);
        result
    }
    ...
    pub(crate) fn call_compiled_method(
        ...
        if crate::runtime::native_methods::any_monitor_class() {
            let inv = invocant.clone();
            return self.with_monitor_lock(receiver_class_name, method_name, inv.as_ref(), |i| {
                i.call_compiled_method_body(
                    receiver_class_name,
                    owner_class,
                    method_name,
                    method_def,
        ...
        self.call_compiled_method_body(
            receiver_class_name,
            owner_class,
            method_name,
            method_def,
    ...
    pub(super) fn call_compiled_method_fast(
        ...
        if crate::runtime::native_methods::any_monitor_class() {
            ...
            return self.with_monitor_lock(receiver_class_name, method_name, Some(&target), |i| {
                i.call_compiled_method_fast_body(
                    receiver_class_name,
                    owner_class,
                    method_name,
                    method_def,
        ...
        self.call_compiled_method_fast_body(
            receiver_class_name,
            owner_class,
            method_name,
            method_def,
    ...
    #[allow(clippy::too_many_arguments)]
    fn call_compiled_method_fast_body(
        &mut self,
        receiver_class_name: &str,
        owner_class: &str,
        method_name: &str,

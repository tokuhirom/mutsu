...
impl VM {
    pub(super) fn try_native_method(
        ...
        if loan_env!(self, mixin_role_has_method(target, &method_name)) {
            return None;
        }
        ...
        if matches!(target, Value::Instance { .. }) {
            if let Value::Instance { class_name, .. } = target {
                ...
                let render_overridden =
                    is_pure_render && loan_env!(self, has_user_method(&cn, &method_name));
                if (!is_pure_render || render_overridden)
                    && (self.type_matches_value("Real", target)
                        || self.type_matches_value("Numeric", target)
                        || loan_env!(self, has_user_method(&cn, "Bridge")))
                {
                    return None;
                }
                // Native method on instance class
                if loan_env!(self, is_native_method(&cn, &method_name)) {
                    return None;
                }
            }
        ...
        if method_name == "decode" {
            return result.map(|res| {
                res.map(|value| match value {
                    Value::Str(decoded) => {
                        Value::str(loan_env!(self, translate_newlines_for_decode(&decoded)))
                    }
                    other => other,
                })
            });
            ...
            && let Some(Ok(v)) = result.take()
        {
            ...
            result = Some(Ok(loan_env!(self, tag_container_metadata(v, info))));
        }

...
impl Interpreter {
    ...
    fn exec_call_method_op_impl(
        ...
        // single Callable block. Calling on the *type object* (`Lock.protect: …`)
        // or passing a non-Callable (`.protect: %()`) matches no candidate and
        // must throw X::Multi::NoMatch, not X::Method::NotFound (roast
        // .../multi-no-match.t).
        if method == "protect" {

    ...
    pub(super) fn exec_apply_var_trait_op(
            ...
            && self.class_does_role(&trait_name, "Associative")
        {
            ...
            if !init_values.is_empty() {
                ...
                let stored =
                    self.call_method_mut_with_values(&name_str, instance, "STORE", vec![list_arg])?;
                self.write_local_slot_or_name(code, eff_slot, &name_str, stored.clone());
                self.set_env_with_main_alias(&name_str, stored);
            }
            return Ok(());
        }

...
impl Compiler {
    ...
    pub(super) fn compile_condition_expr(&mut self, cond: &Expr) {
        match cond {
            Expr::Literal(lit)
                if matches!(
                    lit.view(),
    ...
    fn compile_stmt_inner(&mut self, stmt: &Stmt) {
        ...
        match stmt {
            ...
            Stmt::Block(stmts) => {
                // One lowering, shared with the value position -- see
                // `control_block.rs`.
            ...
            Stmt::Assign {
                ...
                // A plain object/scalar invocant is immutable, but an aggregate
                // invocant such as `@items` is a caller-owned container and
                // Raku lets a method replace it through `self`. Compile the
                // latter through an internal assignment target; the VM checks
                // whether the current implicit invocant arrived with a live
                // aggregate container and rejects every other shape. Outside
                // a method `self` is an ordinary user variable, so this branch
                // must remain method-scoped.
            ...
            Stmt::If {
                ...
                // Everything below is position-independent and lives in
                // `control_if.rs`, shared with the expression form.
                self.compile_if_construct(
                    ...
                    binding_var,
                    *is_statement_modifier,
                    crate::compiler::control_if::IfPosition::Statement,
                );
            }
            Stmt::While {
                cond,
            ...
            Stmt::For {
                ...
                // Everything below is position-independent and lives in
                // `control_for.rs`, shared with the expression form.
                self.compile_for_construct(crate::compiler::control_for::ForParts {
                    iterable,
                    param,
                    param_def: param_def.as_ref(),
                    params,
                    params_def,
                    body,
                    label,
            ...
            Stmt::Call { name, args } => {
                ...
                // TODO: compile it escaping, like the expression form. The
                // container traits a captured cell used to lose are read
                // through it now (#9488); what still blocks it is this
                // statement form's `ExecCallPairs` carrier, which does not
                // write a closure's `:=` rebind of a celled caller lexical
                // back to the caller (roast S02-types/array-shapes.t #29).
                // Retiring `ExecCallPairs` (#9462) removes that carrier.

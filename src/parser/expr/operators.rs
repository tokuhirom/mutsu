...
impl ComparisonOp {
    pub(super) fn token_kind(self) -> TokenKind {
        match self {
            ...
            ComparisonOp::Coll => TokenKind::Ident("coll".to_string()),
            ComparisonOp::Unicmp => TokenKind::Ident("unicmp".to_string()),
...
pub(super) fn parse_multiplicative_op(r: &str) -> Option<(MultiplicativeOp, usize)> {
    ...
    // Unicode: divide (U+00F7) -- skip if user-defined infix exists, and leave
    // `÷=` for the compound-assignment parser.
    if r.starts_with('\u{00F7}')
        && !r.starts_with("\u{00F7}=")
        && !super::super::stmt::simple::is_user_defined_infix("\u{00F7}")
    {

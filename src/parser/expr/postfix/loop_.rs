...
pub(in crate::parser::expr) fn prefix_expr(input: &str) -> PResult<'_, Expr> {
    ...
    if input.starts_with('^') && !input.starts_with("^..") {
        ...
        let parsed_operand = crate::parser::expr::precedence_meta_ops::power_expr_tight(rest)
    ...
    if input.starts_with("|<<") || input.starts_with("|>>") || unicode_hyper_slip {
        ...
        let (rest, _) = ws(&input[marker_len..])?;
    ...
    crate::parser::expr::precedence_meta_ops::power_expr(input)
}
...
pub(in crate::parser::expr) fn postfix_expr_tight_pub(input: &str) -> PResult<'_, Expr> {
        ...
        if rest.starts_with('\u{00BB}') || rest.starts_with(">>") {
            ...
            if !after_hyper.starts_with('.')
                && let Some((full_name, consumed)) =
                    crate::parser::stmt::simple::match_user_declared_postfix_op(after_hyper)
            {
                expr = Expr::HyperMethodCall {
                    target: Box::new(expr),
            ...
            if modifier.is_none()
                && let Some((full_name, consumed)) =
                    crate::parser::stmt::simple::match_user_declared_postfix_op(r)
            {
                let inner = full_name

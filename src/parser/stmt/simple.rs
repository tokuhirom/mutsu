...
/// registry too (#8022), and has no interpreter to consult instead. Same for
/// the user-type registry (`is_user_declared_type`): a lowercase-named user
/// `subset` in a `--> Type` return type must be classified as a type
/// constraint rather than a definite return value, independent of runtime

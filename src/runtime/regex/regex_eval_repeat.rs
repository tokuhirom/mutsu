...
impl Interpreter {
    ...
    /// Reduce-time bottom-up walk over the winning capture tree, children first.
    ///
    /// Every embedded `{ … }` block has already run inline, where the cursor
    /// reached it, so this walk no longer executes anything: what is left is the
    /// per-rule `:my $*x` bookkeeping the `:actions` walk needs — re-establishing
    /// each declaring match's own binding — plus republishing the top node's
    /// `make` value into `env["made"]`, which is where `Grammar.parse` and
    /// smartmatch read the whole match's `.made` from.
    ...
    pub(in crate::runtime) fn reduce_regex_captures_made_for_rule(
        ...
        // the same name would otherwise decide the value.
        self.record_rule_dynvars(caps, &declared_keys);
        // The inline `{ make … }` committed this node's value during matching.
        // Republish it where the call sites read the whole match's `.made` from
        // (`Grammar.parse`, smartmatch); nothing else will.
    ...
    pub(in crate::runtime) fn reduce_cap_node_for_rule(
        ...
        // A declaring match whose value had to be re-derived must record what its
        // binding holds — its action still reads it.
    ...
    fn reduce_child_axes(
        ...
        // The walk's only remaining job is the per-rule `:my $*x` bookkeeping:
        // every embedded `{ … }` block ran inline during matching, and each
        // node's `ast` was committed on the capture delta there. A grammar that
        // declares no dynvar therefore has nothing to do here, and descending
    ...
    /// Re-establish `rule_name`'s own `:my $*x = …;` bindings in the env for the
    /// duration of this node's reduce step, and report the env keys involved.
    ///
    /// The value comes from `recorded` — the node's own `regex_vars`, which the
    /// **match** already filled in: the `:my $*x = …;` declarator evaluated the
    /// initializer per match (`RegexAtom::VarDecl`) and every inline `{ … }`
    /// block of that match threaded its writes through the same carrier, so the
    /// node holds exactly what the variable held when this match ended. Falling
    /// back to re-evaluating the declaration covers the shapes the carrier never
    /// reached (a rule whose declaration is hoisted as a leading declarator, for
    /// instance), and keeps a match from inheriting whatever an earlier match of
    /// the same rule left behind.
    ///
    /// Returns the keys whose value must still be read back off the env
    ...
    /// had to re-derive onto this node, for the action walk (a separate, later
    /// pass) to re-install around this node's action.

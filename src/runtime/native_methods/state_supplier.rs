...
struct BatchState {
    ...
    elems: Option<usize>,
    /// Time window in seconds (from :seconds) — used only for done-flush;
    /// actual timer ticking is handled by a background thread.
    seconds: Option<f64>,
...
pub(in crate::runtime) fn register_supplier_batch_tap(
    ...
    if let Ok(mut map) = supplier_subscriptions_map().lock() {
        map.entry(supplier_id)
            ...
            .taps
            .push(SupplierTapSubscription {
                callback: Value::Nil, // batch taps emit to a downstream supplier
                line_mode: false,
                line_chomp: true,
                line_buffer: String::new(),
                delay_seconds: 0.0,

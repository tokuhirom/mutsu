...
impl Interpreter {
    ...
    // full codepoint count, skip to `$pos`). Rakudo: O(m) -- see #NNNN.
    ...
    // string before the O(k) slice). Rakudo: O(k), k = chars returned -- see #NNNN.
    ...
    // Cost: as `dispatch_substr`: O(n). Rakudo: O(k) -- see #NNNN.

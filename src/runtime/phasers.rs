...
fn lift_phasers_from_stmt(
    ...
    match stmt {
        ...
        Stmt::Block(body) => {
            // A block is "non-transparent" (has variables) if any direct child
            // is a VarDecl, or if any SyntheticBlock child contains a VarDecl.
            // SyntheticBlocks are generated by `my $x will <phaser> { ... }` and
            // similar constructs; they logically introduce a variable into the
            // enclosing scope, so the block should not be treated as transparent.
            let has_var_decls = body.iter().any(|s| match s {
                Stmt::VarDecl { .. } => true,

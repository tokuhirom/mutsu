...
impl Interpreter {
    ...
    pub(crate) fn call_function(
        ...
        // below nor the fallback chain after it. Dispatch it here — the same
        // short-circuit the VM's call opcode applies — so a no-paren
        // `nqp::time` term (which arrives through this entry point) costs a
        // table lookup rather than a full registry walk.
        ...
        match name {
            ...
            "head" => {
                if args.len() < 2 {
                    return Err(RuntimeError::new("Too few positionals passed to 'head'"));
                }
                let head_spec = args[0].clone();
                let arg_sources = self.pending_call_arg_sources.clone().unwrap_or_default();
                let mut values = Vec::new();
                for (offset, value) in args[1..].iter().enumerate() {
                    let source_name = arg_sources
            ...
            // The core list routine. `Test`'s `skip($reason?, $count = 1)` is an
            // ordinary imported sub from the vendored `Test.rakumod` and is
            // resolved before this arm; the two used to be disambiguated here by
            // `skip_call_is_list_skip` because one of them was a native handler
            // (#7566).
            "skip" => self.builtin_skip(&args),

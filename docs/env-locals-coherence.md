...
- **Stage 0 — outer 読み/書きチョークポイント棚卸し（挙動不変）**: slot コンテナの read（GetLocal/GetGlobal/
  ...
  - **[~] 差分 probe で炙り出した残 Stage 0 gap（2026-06-18・着手前に解くべき設計込み）**: `my`/`our` ×
    ...
    1. **bound hash の whole 再代入 `my %a := %b; %a = (z=>9)` が "Cannot assign to a readonly variable (%a)"**
       （raku は %b も (z) に置換）。array は動く（`@a := @b; @a = (9,9)` は伝播）＝**hash 特有の非対称**。root-cause は
       **`readonly_vars` が 3 概念を混同**: ①`:=` bind signal ②`constant %M` の不変性 ③`is readonly` param。
       要素 path（`is_bound_hash_var = readonly && %`）は readonly % を**全て bound（writable・in-place）扱い**
       ＝逆に `constant %M<a>=9` が誤って成功する**対称な既存バグ**を生む。whole-reassign path（コンパイラが emit する
       `CheckReadOnly` opcode＝`vm.rs:4408`）は readonly % を**全て immutable 扱い**＝bound の whole-reassign を阻む。
       **`constant %M` は mutsu では plain Hash（`.^name`=Hash・declared_type≠Map）で bound hash と型で区別不可能**＝
       CheckReadOnly を bound 用にスキップすると constant/Map の whole-reassign 保護（現在 raku 同様に error）も壊れる。
       **正攻法＝`:=` bound var に readonly とは別の専用マーカー（`__mutsu_bound::%a`）を立て、element/whole 両 path を
       それで分岐**（or `constant %M` を真の Map 化）。中規模。次セッション候補。
    ...
    3. **bound array の for-rw topic 非書き戻し**: `my @a := @b; for @a { $_++ }; say @b` が mutsu 不変 / raku は加算。
       bound array を for rw で回したとき topic 書き戻しが bound source に届かない。
    - これらは Stage 1（outer cell 化）が構造的に解く候補だが、2/3 は per-bug の aliasing 修正でも対処可。1 は
      `readonly_vars` 分離が前提＝Stage 1 と独立に進められる中規模リファクタ。

# TRIAGE — prioritized snapshot of the open-findings backlog

**This is the last file-based snapshot, carried over unchanged at the
2026-09-08 migration from `todo/` to GitHub issues.** Its analysis is preserved
in full because it is the most complete survey the backlog has; its *rows* are
dated 2026-09-07b and were already drifting when it was written.

Read it accordingly:

- Every finding still open at the migration links to its issue. A row rendered
  as plain text with *(closed before the migration)* was resolved before the
  move — look for `news/YYYY-MM/<slug>.md`, which keeps the same slug.
...
editing this file. Regenerate the whole thing when it has drifted too far:
re-survey every open `todo:ticket` / `todo:deep` / `todo:perf` issue, re-run
each runnable repro against the `raku` oracle on a fresh build, re-score, and
rewrite — then reconcile the `tier:*` labels with the result. The next regen
should replace the historical sections below outright rather than patching
them.

---

## Coverage gap — 17 open issues this snapshot never surveyed

These were filed after the survey commit, or were skipped by it. They carry no
tier, and nothing below says anything about them. **Start the next regen here**,
then re-verify the ranked sections.

- [#7538](https://github.com/tokuhirom/mutsu/issues/7538) — object-hash-composite-key-increment-does-not-accumulate
- [#7554](https://github.com/tokuhirom/mutsu/issues/7554) — vendor-real-test-module-flip
- [#7555](https://github.com/tokuhirom/mutsu/issues/7555) — vendored-test-battery-gate-regressions
- [#7565](https://github.com/tokuhirom/mutsu/issues/7565) — use-test-taxes-every-hot-loop-in-the-file
- [#7566](https://github.com/tokuhirom/mutsu/issues/7566) — retire-the-native-test-provider
- [#7569](https://github.com/tokuhirom/mutsu/issues/7569) — unify-block-statement-and-value-compilation
- [#7575](https://github.com/tokuhirom/mutsu/issues/7575) — regex-inline-code-carrier-prologue-overhead
- [#7577](https://github.com/tokuhirom/mutsu/issues/7577) — big-denominator-rat-str-truncates-to-f64
- [#7580](https://github.com/tokuhirom/mutsu/issues/7580) — block-scoped-use-drops-a-nested-modules-imports
- [#7583](https://github.com/tokuhirom/mutsu/issues/7583) — do-block-tail-declaration-drops-its-trait
- [#7585](https://github.com/tokuhirom/mutsu/issues/7585) — nested-block-state-not-reset-in-value-position-for
- [#7586](https://github.com/tokuhirom/mutsu/issues/7586) — hyper-assign-to-a-list-of-lvalues-cannot-broadcast
- [#7587](https://github.com/tokuhirom/mutsu/issues/7587) — code-object-renders-as-nothing-inside-a-list
- [#7589](https://github.com/tokuhirom/mutsu/issues/7589) — one-element-slice-assignment-rvalue-is-not-a-list
- [#7592](https://github.com/tokuhirom/mutsu/issues/7592) — next-phaser-clobbers-value-position-for-result
- [#7593](https://github.com/tokuhirom/mutsu/issues/7593) — regex-assertion-scalar-write-to-outer-lexical-is-lost
- [#7594](https://github.com/tokuhirom/mutsu/issues/7594) — one-element-array-raku-omits-comma-for-subclass-element
...
— see [lazy-seq-argument-vanishes-into-a-user-slurpy](https://github.com/tokuhirom/mutsu/issues/7591),
...
| [ADR-0039](../docs/adr/0039-container-lexicals-resolve-lexically.md) container lexicals resolve lexically | Slice 1 landed 2026-08-20; **slice 2 attempted and withdrawn a THIRD time on 2026-09-07**. Only repair 5 shipped (`442a3a993`, under a PR titled "slice2"). | #7541, which is now an accurate same-day withdrawal record. |
| [ADR-0073](../docs/adr/0073-regex-atom-candidates-are-demand-driven.md) regex atom candidates on demand | **`Proposed`; slices 1 and 3 implemented, and slice 2's *ratcheted* half implemented** 2026-09-07. Slice 2's non-ratcheted half is open. | #7548 **is** that open half. The ADR and the todo file agree exactly — unusual here, and worth trusting. |
...
| [ADR-0070](../docs/adr/0070-native-methods-declare-the-named-arguments-they-accept.md) native methods declare accepted nameds | **Header says "slice 1"; its own §Implementation records slice 2 landed 2026-09-07.** Sweep is at **18 of 1 422** probes (raku's own baseline is 23). | #7544 — hardening, not a live bug. |
...
- **Three new bugs found during this regen, each reproduced independently
  ...
  - [channel-supply-tap-done-callback-never-fires](https://github.com/tokuhirom/mutsu/issues/7584)
    ...
    implementations.
  - [no-pseudostash-type-caller-stash-reports-stash](https://github.com/tokuhirom/mutsu/issues/7588)
  ...
  - [nativecall-type-table-shadows-a-user-class-of-the-same-name](https://github.com/tokuhirom/mutsu/issues/7582)
    ...
    #7560 recorded and declined to file.
...
- **`docs/doc-diff-backlog.md` was re-swept and rewritten on 2026-09-07b — the
  ...
  **nine real bugs were hiding under that noise**. Tracked in
  [doc-diff-harness-has-no-output-cap-or-nondeterminism-gate](https://github.com/tokuhirom/mutsu/issues/7590);
...
- **`todo/` files whose own root-cause or status section is wrong** — still the
  ...
  #7573 (third consecutive framing dead —

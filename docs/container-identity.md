...
- [~] **Stage 2 — 深い `>>++`/`deepmap`/object-hash・既存 SlotRef 機構の撤去**（着手 2026-06-13、スライス進行中）:
  ...
  - **残スライスのマップ** — 詳細設計は **`docs/slotref-removal-plan.md`**（cells-everywhere
    課題の役割分離・WriteCursor 化・phantom-entry・hazard 監査・段階スライスを固めた専用 doc）:
    1. **hash_autovivify / 非 lazy 連鎖の SlotRef 撤去**: write-autoviv 連鎖（`%h<a><b> = 5`）の中間/終端 slot-ref。
       終端 assign 先の表現を要再設計（毎 write の cell 化は cells-everywhere になるため、終端は呼び出し側で
       書き戻す形が候補 = 非 Value の WriteCursor、plan §3.1）。
    2. **hash_slot_ref_lazy / DeferredHashAccess（missing-key lazy bind）**: `:exists` を汚さない遅延束縛が要件。
       cell 化するなら「phantom entry」相当の設計が必要 — 最後に回す（plan §3.3）。
    3. **grep-rw-view 撤去**: 登録ゲート下で matched 要素を cell 昇格し view registry（最後の ptr-keyed
       グローバルの一つ）を全廃する案。for-loop rw topic (vm_control_ops ~410/613) が消費者。
    4. 全生成が死んだら consumer 群（ArraySlotRef 40 / HashSlotRef 62 / DeferredHashAccess 31 refs）と
       variant 自体を削除（plan §4 slice 5）。

...
- [~] **Stage 2 — 深い `>>++`/`deepmap`/object-hash・既存 SlotRef 機構の撤去**（着手 2026-06-13、スライス進行中）:
  ...
  - **残スライスのマップ**:
    1. **BOUND_HASH_REF_SENTINEL 撤去**: 生成 3 経路（単一 hash-key bind `%h<k> := $var` の named handler
       vm_var_assign ~3199・slice bind ~2869・`.BIND-KEY` vm_call_method_mut_ops ~1095/1128）を array 側 #2914 と
       同型の「cell を hash entry に格納 + source 変数へ同 cell 書き戻し（pending_source_cell パターン）」へ。
       その後 consumer（vm_var_ops 94/114/127 等の read/write resolve）撤去。
    2. **BOUND_ARRAY_REF_SENTINEL 撤去**: 残る生成は element-source bind（`\0idx\0` encoded、vm_var_assign ~4263 の
       ArraySlotRef 設置と対）。#2922 の terminal-promotion を named 単一レベル handler の element-element にも適用。
    3. **hash_autovivify / 非 lazy 連鎖の SlotRef 撤去**: write-autoviv 連鎖（`%h<a><b> = 5`）の中間/終端 slot-ref。
       終端 assign 先の表現を要再設計（毎 write の cell 化は cells-everywhere になるため、終端は呼び出し側で
       書き戻す形が候補）。
    4. **hash_slot_ref_lazy / DeferredHashAccess（missing-key lazy bind）**: `:exists` を汚さない遅延束縛が要件。
       cell 化するなら「phantom entry」相当の設計が必要 — 最後に回す。
    5. **grep-rw-view 撤去**: 登録ゲート下で matched 要素を cell 昇格し view registry（最後の ptr-keyed
       グローバルの一つ）を全廃する案。for-loop rw topic (vm_control_ops ~410/613) が消費者。
    6. 全生成が死んだら consumer 群（ArraySlotRef 41 / HashSlotRef 62 / DeferredHashAccess 31 refs）と
       variant 自体を削除。

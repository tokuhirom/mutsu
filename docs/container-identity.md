...
- [ ] **Stage 0 — write/read チョークポイントの確立（挙動不変・byte-identical）**: 散在する `arr[i]=v` を
  単一ヘルパ `Value::assign_element_slot(slot, val)`（ContainerRef なら write-through、でなければ置換）へ機械的に
  ルーティング（cell が無い現状は plain 置換＝挙動不変）。要素 read も `resolve_array_entry` + slice/iteration の
  各箇所を decont ヘルパ経由に集約。**この PR は観測挙動ゼロ変化＝ CI でほぼ確実に緑**。Phase 3 Stage 0 の
  「`InstanceAttrs` カプセル化境界」と同型の地ならし。
- [ ] **Stage 1 — 束縛要素のセル切替（局所スイッチ）**: `array_slot_ref`/`hash_slot_ref`/`DeferredHashAccess`
  生成を `ContainerRef` 昇格へ変更（`$x` も同じ cell の ContainerRef を持つ＝Phase 1 のスカラー cell 機構を再利用）。
  Stage 0 で write/read が全部チョークポイントを通っているので、ここは**局所変更で leak しない**。解ける:
  nested.t 深い束縛, take-rw, `@a[0]:=`。**escape-aware**: セル昇格は `:=` 束縛時のみ＝通常配列は bare 値のまま
  （perf 崖を構造的に回避。#2746 の轍を踏まない）。
- [ ] **Stage 2 — 深い `>>++`/`deepmap`/object-hash・既存 SlotRef 機構の撤去**: cell が要素 identity の単一源に
  なったら `ArraySlotRef`/`HashSlotRef`/`DeferredHashAccess`/`BOUND_*_REF_SENTINEL`/grep-rw-view binding を全廃。

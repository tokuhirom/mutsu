...
- [~] **Stage 2 — 深い `>>++`/`deepmap`/object-hash・既存 SlotRef 機構の撤去**（着手 2026-06-13、スライス進行中）:
  ...
  - **残スライスのマップ** — 詳細設計は **`docs/slotref-removal-plan.md`**（cells-everywhere
    ...
    1. **残 `HashSlotRef` 生成（entry 作る系）の cell 化**: `hash_autovivify`（junction-key bind 97 /
       `is raw` reduce lvalue-read 811-868 / autoviv-op fallback）は呼び出し後に entry 存在＝phantom 不要 →
       cell 化可（hot path 非該当ゆえ崖なし）。deferred-token 用途以外の `HashSlotRef` を枯らす（plan §4 slice 4）。
    2. **grep-rw-view 撤去**: 登録ゲート下で matched 要素を cell 昇格し view registry（最後の ptr-keyed

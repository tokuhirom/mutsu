...
- [ ] **次スライス = `is rw` param 真の共有セル（scalars.t 24/27）**: 実装プラン = `docs/is-rw-shared-cell-plan.md`
  （ターゲット・repro A-C・root cause = writeback vs live alias・サイト一覧・gating・落とし穴・検証手順を埋め込み済み）。
  `is rw` param を value-writeback（`apply_rw_bindings_to_env`）から live `ContainerRef` cell へ。readonly param は
  snapshot 維持（promotion は `is_rw` でゲート）。ホットな call path のため高リスク。

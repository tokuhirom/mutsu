...
- [~] **Stage 2 — 深い `>>++`/`deepmap`/object-hash・既存 SlotRef 機構の撤去**（着手 2026-06-13、スライス進行中）:
  ...
  - **残スライスのマップ** — 詳細設計は **`docs/slotref-removal-plan.md`**（cells-everywhere
    ...
    1. **欠落 key terminal bind（`hash_slot_ref` None arm 2918）→ phantom-entry cell**（plan §3.3 前哨）。
    2. **hash_slot_ref_lazy / DeferredHashAccess（missing-key lazy bind）**: `:exists` を汚さない遅延束縛が要件。
       cell 化するなら「phantom entry」相当の設計が必要（plan §3.3）。
    ...
    4. **残 `HashSlotRef` 生成（junction-key bind 97 / `is raw` reduce の lvalue-read autoviv 811-868）を
       cell 化** → 全生成が死んだら `HashSlotRef`/`DeferredHashAccess` variant + helper を一括削除（plan §4 slice 5）。

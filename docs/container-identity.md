...
- [~] **Stage 0+1 — 配列要素の束縛セル化（着手・RHS 束縛 landed）**: Stage 0（write チョークポイント）と
  ...
  - [x] **深い LHS 束縛の cell 化（landed・Stage 2 着手）**: `$struct[..]<..>[..] := $scalar`（`IndexAssignDeepNested`
    ...
    - **残**: ①単一レベルも cell へ移行 → `BOUND_ARRAY_REF_SENTINEL` 削除（次スライス）。②`<key>()` を挟む LHS 束縛
      （nested.t 11-12、`IndexAssignGeneric`/method-lvalue 経路）。③element-element 束縛（`$a[i]:=$b[j]`、nested.t 32-33/35-37、
      `\0idx\0` encoded source）。

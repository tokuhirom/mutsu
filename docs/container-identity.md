...
- [~] **Stage 0+1 — 配列要素の束縛セル化（着手・RHS 束縛 landed）**: Stage 0（write チョークポイント）と
  ...
  - [x] **ハッシュ要素 Stage 0 — write チョークポイント確立（挙動不変・landed）**: 全 Raku-hash-value の
    ...
    維持、`make test` 緑、`hash_chokepoint_tests` 3 ケース）。**次スライス = Stage 1（昇格）**: `array_slot_ref` の
    scalar-leaf 昇格を hash 版（`hash_autovivify`/`hash_slot_ref_lazy`）へ展開し `:=` 束縛 hash 要素を ContainerRef 化。
    read は `resolve_hash_entry`（既に decont 済み）＋ slice/iteration/.raku/native の raw items 読みの透過化が要る。
  - **LHS 配列束縛（`@a[i]:=$var`）の所見**: 単一レベルは現状 sentinel で動く。深い `$struct[..][1]:=$abbrev` は
    `$abbrev=X→element` 方向が深さで stale。cell 化には **scalar var → cell 昇格**（$abbrev のスロットに ContainerRef を
    書き戻す）＋ element に同 cell を格納、が要る（RHS の対称形）。array-only なので hash 干渉は無いが、bind_source 経路
    （複数 write site）の sentinel を cell へ切替える必要。

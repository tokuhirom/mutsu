...
| 機構 | 役割 | 削除タイミング |
|------|------|--------------|
| `owned_captures`（`vm_register_ops.rs` `compute_owned_captures`） | ループ per-iteration 値凍結 | step 2 |
| `closure_captured_state`（per-instance 値凍結 + writeback） | 返却クロージャの state 維持 | step 2 |
| `box_captured_lexicals` の `loop_local_vars` 経路 | ループ本体 boxing | step 1→2 |

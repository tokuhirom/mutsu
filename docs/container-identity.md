...
| 機構 | 役割 | 削除タイミング |
|------|------|--------------|
| `owned_captures`（`vm_register_ops.rs` `compute_owned_captures`） | ループ per-iteration 値凍結 | step 2（残） |
| `closure_captured_state`（per-instance 値凍結 + writeback） | 返却クロージャの state 維持 | **✅ step 2 で削除（下記）** |
| `box_captured_lexicals` の `loop_local_vars` 経路 | ループ本体 boxing | **捕捉時が正位置＝据置**（下記教訓） |
| `multi_captured_mutated_locals`（#2751） | 兄弟クロージャ boxing（暫定） | step 1 が subsume |

env snapshot（`clone_env`/`flattened`）は捕捉の土台として残るが、捕捉値が裸値かセル（Arc 共有）かは
escape 解析が決める。
...
- 2026-06-08: **step 2 = `closure_captured_state` を削除（4→3）**。
  - **試して revert した方向（重要教訓）**: 「`box_captured_lexicals`（捕捉時セル化）を宣言時へ再配置して削除」を試作。
    捕捉ローカルは block/loop/sub で sigil 無し名（`a`）＝`simple=false`→slow path を通る等を解明し動かしたが、
    **`S04-blocks-and-statements/let.t` を決定的に回帰**（`let`/`temp` 復元が壊れた）。宣言時セル化は `let`-save より前に
    セル化するため、save がセル Arc を保存し復元できない。**`box_captured_lexicals` は「捕捉時」という正位置にある**
    （宣言時機構＝let/temp save・型チェック・slow-path store の後に box することで干渉回避）。セル**生成**の宣言時移設は誤り。
    `make test` では露見せず **release roast でのみ**判明（前回同様）。revert。
  - **採った方向＝実機構削除**: step 1 の escape 解析が `my $inc=->{++$x}`・factory `sub mk{my $n=0;sub{$n++}}` 等の
    **代入/返却クロージャを escaping と判定→捕捉変異変数をセル化**するようになったため、`closure_captured_state`
    （per-instance 値永続の副テーブル `HashMap<(closure_id, Symbol), Value>`）が**冗長**になったと仮説。
    実証: read/write を無効化→`make test` PASS（5336）、非脱出 multi-call `{++$x}`×2=2・factory 独立状態=010、
    closure-captured-state.t PASS、**release で 166 state-sensitive ファイル 0-fail**・scheduler/every.t/state.t/closure.t PASS。
    → **field・accessors（get/set）・read override・write persist を完全削除**（-45 行）。per-instance 独立性は「factory 呼び
    ごとに新しい宣言＝新しいセル、兄弟は同一セル共有」とセルモデルから自然に出る。`box_captured_lexicals`・`owned_captures`
    （read-only ループ値凍結）は据置（セルが未カバー、削除は別途要設計）。
  - 検証: `make test` PASS（5336）・clippy clean・int.t release 0.07s・上記 release スイープ。**最終判定は CI release roast**。
  - **教訓**: step 2 の本丸は box 再配置ではなく**値凍結副テーブルの削除**。escape 解析（step 1）がセル化を広げた分、
    per-instance 副テーブルが不要化する。owned_captures（per-iteration 値凍結）も同様に検証する価値あり（ただし read-only
    ループ捕捉はまだ非セルなので別。次スライス候補）。

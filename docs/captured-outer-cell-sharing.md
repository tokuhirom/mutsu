...
- **`S04-statements/lazy.t`**（**新規同定・pre-existing**）= double-OFF で **hang**（exit 124・proper harness でも・
  baseline でも同 hang＝S19 と無関係）。**精密切り分け済**＝**`use Test`（END phaser）+ scope 内に `Value::LazyThunk` が存在**
  する組み合わせだけで hang。**最小再現**＝`use Test; plan 1; my $var := lazy { 42 }; ok 1;`（force も capture も不要・出力後に
  終了/END フェーズで hang）。切り分け: `lazy` 単独=OK / `use Test` 単独=OK / `use Test`+array=OK / `lazy`+`LEAVE`（Test 無し）=OK
  → **`use Test` の END/plan-check フェーズが LazyThunk を含む env を double-OFF で処理して loop/deadlock**。gather でなく
  `lazy { block }`＝別機構。§10.19 の「残 2」が見落とし（handoff survey が proper harness でなく lazy-lists/throttle のみ計測した
  疑い）。**次の最有力 double-OFF blocker**＝END-phase の env↔locals 処理が LazyThunk で収束しない点を特定要。

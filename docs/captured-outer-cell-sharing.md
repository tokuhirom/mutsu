...
### 10.20 slice S16（2026-06-22）— proto-multi 候補の captured-outer writeback を precise 化（roast 5→4）
...
### 10.19 ハンドオフ — roast double-OFF 残 4（S16 後・2026-06-22）

S4〜S16（14 slice）で **roast double-OFF surface 25 → 4**。全 whitelist（1285）の double-OFF sweep
（`MUTSU_NO_BLANKET_RECONCILE=1 MUTSU_NO_PRECISE_RECONCILE=1` release）で確定した残 4（新規回帰ゼロ）:

| file | サブテスト | 種別 | 次の一手 |
|---|---|---|---|
| `S12-meta/primitives.t` | 9 | **writeback 候補** | 自作 `Metamodel::Primitives.create_type` HOW の `method type_check { $union-type-checks++ }` が captured counter を変異。type-check dispatch 経路。slot-only か closure-vs-direct で判定 → snapshot 差分。 |
...
**∴ 次セッションの着手**: ①primitives（writeback 候補・snapshot 差分で解ける見込み）→ ②③④は別軸（laziness/timing/parser）で
env_dirty 削除の前提から外せる可能性を精査（特に throttle は flaky・terminator は parser）。**primitives を畳めば、別軸 3 件の
扱いを決めて §7.4 / PLAN §2-E（`env_dirty` 物理削除）に進める。** slot-only 判定法＝同変数を `{ $x }()`（closure＝env 読み）と
direct（slot 読み）で比較し差があれば slot-only＝snapshot 差分で解ける。診断ファイル＝`tmp/roast-double-off-final-fails.txt`。
**★S16 の教訓**: proto-multi 候補は **常に slow path（`run_instance_method_resolved`）経由**で、この path は捕捉 write を
env に伝播するが `env_dirty` を立てない＝blanket pull すら発火せず **default build でも slot が stale**（`is` 読みは
reconcile site を踏むので roast は偶然 green だが `say` 直読みは壊れる）。∴ S16 の修正は **ungated**（precise writeback は
blanket の部分集合で正しい結果を変えず、default の潜在バグも直す）。primitives も HOW メソッドが slow path 経由なら同型。

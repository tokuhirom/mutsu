...
**★残 double-OFF サーフェス（S20 後）= 1（throttle・timing flaky のみ）＋ writeback 候補ゼロ**。§10.23 の
full-consumption-through-pipe gather tail writeback は roast 非該当（lazy-lists は slice）で残るが決定的 roast blocker でない。
∴ **env_dirty 物理削除（§7.4 / PLAN §2-E）の決定的 roast blocker は実質ゼロ**（throttle が flaky で決定的 pin 不可なら除外可）。
次セッション＝throttle が flaky か最終確認 → env_dirty 物理削除着手。

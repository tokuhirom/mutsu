...
### 10.19 ハンドオフ — roast double-OFF 残 5（2026-06-22 authoritative sweep 確定）

S4〜S15（13 slice）で **roast double-OFF surface 25 → 5**。全 whitelist（1285）の double-OFF sweep
（`MUTSU_NO_BLANKET_RECONCILE=1 MUTSU_NO_PRECISE_RECONCILE=1` release）で確定した残 5（新規回帰ゼロ）:

| file | サブテスト | 種別 | 次の一手 |
|---|---|---|---|
| `S12-methods/defer-next.t` | 15,17 | **writeback 候補** | multi-method `&?ROUTINE.dispatcher()` deferral の captured `$r`。minimal repro は PASS＝full test の特定候補構造のエッジケース。CallMethod 系の deferral 経路で snapshot 差分が要る箇所を特定する。 |
...
**∴ 次セッションの着手**: ①defer-next ②primitives（writeback 候補・snapshot 差分で解ける見込み）→ ③④⑤は別軸（laziness/timing/parser）で
env_dirty 削除の前提から外せる可能性を精査（特に throttle は flaky・terminator は parser）。**writeback 2 件を畳めば、別軸 3 件の

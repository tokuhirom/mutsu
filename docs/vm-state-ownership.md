...
- **`current_package` 共有ハンドル移管（2026-06-12, registry 撤去 enabler）**: registry の VM ネイティブ dispatch read
  ...
  - **次（registry 撤去の本payoff）**: `Registry::has_proto(current_package, name)` 等を pure `impl Registry` メソッド化し、
    VM dispatch を `self.registry` + `self.current_package()` で native 読みへ寄せる（interpreter wrapper の VM 経路を撤去）。
    本 PR がその前提。

...
### 3.1 役割 A: write-autoviv カーソルを非 Value 化（WriteCursor）
...
1. **slice 1 — bind source slot 4256/4291 の到達性確定 + 整理**
   到達性を実測 → デッドなら削除 / 現役なら cell 化（§3.2）。最小・低リスク。
   pin: `t/array-bind-cell.t` / `t/hash-bind-cell.t` 既存。

2. **slice 2 — write-autoviv カーソルの非 Value 化（§3.1 案 A1）**
   `hash_autovivify` を `WriteCursor` 戻りへ。`exec_index_autovivify_op` と後続
   assign op を融合 or VM フィールド退避。**最ホット経路** — 必ず `int.t` wall-clock を
   debug build で counter 確認、release で最終測定。array autoviv arm
   （`array_slot_ref(_,false)`）も同カーソルへ統合し、bind 専用 `array_slot_ref(_,true)` と
   分離。pin: 新規 `t/write-autoviv-cursor.t`（`%h<a><b><c> = 5`・`@a[2][3] = 9`・
   mixed `%h<a>[1]<b>` の深い連鎖）。
...
5. **slice 5 — variant・consumer・helper の一括削除**
   全生成が枯れたら `HashSlotRef`/`ArraySlotRef`/`DeferredHashAccess` を enum から削除。
   連動削除: `hash_slot_read/write`・`array_slot_read/write`・`deferred_hash_read/write`・
   `hash_slot_ref_lazy`・serde arm（serde_support.rs:309-311）・display arm
   （display.rs）・truthy/isa arm（types.rs:479/480/611/612）・introspect
   （methods_introspect.rs）・vm_env_helpers.rs:304 の skip arm・
   vm_var_assign_ops.rs の 4 consumer 群。`cargo build` のコンパイルエラーが網羅的
   チェックリストになる。

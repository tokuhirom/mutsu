...
> ### ✅ 達成（2026-06-23, slice 5）— ただし「全 variant 削除」は **不可能**と確定
>
> 「全 `SlotRef` variant を削除」は **anti-goal の side-table 無しには原理的に不可能**
> （slice 3 注記の再確認）。欠落 key への `:=` bind（`$b := %h<x>`）は、`:exists` を
> 汚さず iteration からも見えないまま「読むと Any・書くと親 hash に materialize」する
> **deferred-vivification token** を要求する。この token は必ず `(hash Arc, key path)` を
> どこかに保持せねばならず、`Value` slot に載る以上それは Value variant でしかありえない
> （`ContainerRef` cell には載らない＝cell は materialize 済みエントリの alias 専用）。
> `is raw` reduce の lvalue 降下（`roast/S32-list/reduce.t`）も同 token を共有する。
>
> ∴ DOD #1 は「`SlotRef` という**概念と名前**を払拭する」と解釈し直した。具体的には
> **`HashSlotRef` + `DeferredHashAccess` の 2 variant を、単一の path ベース
> `Value::HashEntryRef { hash, path }` に統合**した（`parent_slot` チェーン廃止・
> read/write/materialize helper を統合・dead な `hash_slot_ref_lazy` 削除）。これにより
> `git grep 'SlotRef\|DeferredHashAccess' src/` は実際にゼロになり、残る唯一の deferred
> token は「stale な back-reference ではなく vivification token」という正しい名前を持つ。
> 副産物として **3 段以上の全欠落 path bind（`$b := %h<a><b><c>`）が動くようになった**
> （旧 2-variant チェーンは 2 段までしか扱えず raku と相違していた）。pin =
> `t/hash-entry-ref-deep-bind.t`（9）。挙動は `t/phantom-entry-bind.t` で従来どおり担保。
...
3. **slice 3 — phantom-entry: 欠落 key bind の materialize を cell 化【DONE】**
   欠落 key の `:=` bind（`$e := %m<solo>` の `HashSlotRef`、`$d := %k<p><q>` の
   `DeferredHashAccess`）の **最初の書き込みで** 共有 `ContainerRef` cell へ materialize する。
   bound var local を cell へ置換し、終端 hash entry にも同 cell を格納 → 以後 bound var と
   hash entry が双方向 alias（旧 plain-value materialize は alias を失い、materialize 後の
   cross-write が届かない **case-C バグ**だった）。raku 完全一致（lazy セマンティクス case A/B/D
   は不変＝deferred token は最初の write まで保持）。`materialize_bound_slot_to_cell` VM ヘルパ
   を 2 つの SetLocal サイトから呼ぶ。dead 化した `deferred_hash_write` メソッド削除。
   pin `t/phantom-entry-bind.t`(12)。
   **注**: deferred token（`HashSlotRef`/`DeferredHashAccess`）自体は bind〜first-write の遅延
   状態として残る（read=Any・`:exists` 非汚染）。token を Value から完全に消すには「map に入れても
   `:exists`/iteration から見えない phantom 表現」が要る（intermediate も lazy なので tombstone
   不可）→ side-table（ptr-keyed global 復活＝anti-goal）なしには不可。よって**token 残置が正解**で、
   materialize の cell 化（＝本 slice）が phantom-entry の実質本体。

4. **slice 4 — bind-descent の `hash_autovivify` を cell/shared-Arc 化【DONE】**
   `exec_index_autovivify_op` の bind-descent 3 サイト（junction-bind 97 / hash arm 113 /
   slot-ref-target 140）を新メソッド `hash_autovivify_cell` へ。array arm（`array_slot_ref(_,false)`、
   slice 2）と対称に、existing cell→そのまま / container leaf→shared Arc 値 / scalar leaf→cell 昇格 /
   missing→empty Hash autoviv（shared Arc）。`HashSlotRef` back-ref を撲滅し COW staleness も解消。
   挙動完全不変（raku 一致）。既存 bind/junction/phantom suite + roast で担保。
   **scope**: bind-descent のみ。`is raw` reduce lvalue-read（vm_var_index_ops:807/822/864）は**意図的に
   旧 `hash_autovivify` のまま**＝missing key を **Any として読む writable slot** 意味論が必要で、
   `hash_autovivify_cell` の empty-Hash 意味論と異なる（変えると reduce 回帰）。旧メソッドは reduce 用に残る。
   bind/reduce 系 `hash_autovivify` は raku 比較で**バグなし＝純粋 refactor**（campaign の cell 統一目標に整合）。
   調査中に発見した別系統の実バグ（nested/computed target を介した **outermost junction/slice 代入**が key を
   stringify して壊れる）は独立 PR #2981 で修正済み（`exec_index_assign_{expr_nested,deep_nested,generic}_op` に
   autothread 追加、`t/nested-junction-slice-assign.t`）。

5. **slice 5 — 2 variant を単一 `HashEntryRef { hash, path }` に統合【DONE, 2026-06-23】**
   §0 の達成注記参照。`is raw` reduce の lvalue-read autoviv（`hash_autovivify`）は **削除**
   ではなく **path 長 1 の `HashEntryRef` を返すよう変更**（roast/S32-list/reduce.t は現 rakudo
   自身が `assign requires a concrete object` でエラーにする寛容パスだが、mutsu は従来挙動を維持）。
   `DeferredHashAccess` の `parent_slot` チェーンは `path: Vec<String>` に畳まれ、欠落 key bind の
   降下は `exec_index_autovivify_lazy_op` が 1 subscript ずつ path を push する。連動削除: 
   `hash_slot_read/write`→`hash_entry_read/write`（+ 共有 walk-create helper `hash_entry_terminal`）・
   `deferred_hash_read` 削除・dead `hash_slot_ref_lazy` 削除・serde/display/truthy/isa/introspect/
   utils/vm_env_helpers skip arm/vm_var_assign_ops consumer 群を単一 arm に統合。`cargo build` が
   網羅チェックリスト。`make test` 全緑 + binding/reduce 系 roast PASS。
   **value_is_defined / truthy は unmaterialized token を無条件 `false`**（旧 DeferredHashAccess に
   合わせ、かつ単一 key も raku 一致＝外部書き込みで retro-bind しない）— read-through すると
   `t/phantom-entry-bind.t` の retro-bind 判定が壊れる。display/type/isa は read-through（旧 HashSlotRef 相当）。

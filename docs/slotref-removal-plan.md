...
4. **slice 4 — 残 `HashSlotRef` 生成（entry 作る系）の cell 化 + variant 削減**
   `hash_autovivify`（junction-key bind 97 / `is raw` reduce lvalue-read 811-868 / autoviv-op
   fallback）は呼び出し後に entry が存在する＝phantom 不要 → `ContainerRef` cell 化できる
   （bind/junction/reduce 限定で hot path 非該当ゆえ cells-everywhere 崖にならない）。これで
   `HashSlotRef` の deferred-token 用途以外を枯らす。**残置**: deferred-token（slice 3 の理由）。
   **調査結果（2026-06-13）**: これらの `hash_autovivify` サイト（bind/reduce 系）は raku 比較で
   **バグなし＝純粋 refactor で対応実バグ無し**かつ variant 完全削除にも至らない（token 残置）ため、
   **当面 deferred**（低 ROI）。代わりに調査中に発見した別系統の実バグ（nested/computed target を介した
   **outermost junction/slice 代入**が key を stringify して壊れる）を独立 PR で修正
   （`exec_index_assign_{expr_nested,deep_nested,generic}_op` に autothread 追加、`t/nested-junction-slice-assign.t`）。

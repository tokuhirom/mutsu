...
- **Date:** 2026-09-06 (full re-sweep) · debug `mutsu` at `5885101fc` (main,
  through PR #7379) plus the then-unmerged PR #7380 (ADR-0067's
  subscript-receiver producer), which is what the working tree held
- **444 files scanned · 66 have signal**
- **match = 2376 · output-mismatch = 74 · mutsu-crash = 34 · raku-drift = 119**
- High-signal total (mismatch + crash) = **108**, down from **296** on 2026-08-22
  (mismatch 184 → 74, crash 112 → 34, signal files 124 → 66) and **361** on
  2026-07-22. `match` rose 2195 → 2376 over the same period: the blocks are not
  disappearing from the corpus, they are being answered correctly. `raku-drift`
  is flat (120 → 119), exactly as expected — that bucket tracks rakudo moving
  away from the docs, not mutsu.
- The drop reflects the batch-1/batch-2 ticketing rounds *plus* the
  `todo/tickets` + `todo/deep` drain of 2026-08 and 2026-09, most of which was
  driven by findings originally surfaced here.

### Previous snapshot (2026-08-22, for the delta above)

- 444 files scanned · 124 have signal · debug `mutsu` at main ≈ `7d45c5939`
...
| `Type/Any.rakudoc:1549,1559` | ~~`.snitch` (v6.e.PREVIEW debugging method) is entirely unimplemented~~ (method and both subroutine forms; the writable `.snitch = ` form needs a native method to return an lvalue container — [split off](../todo/tickets/native-method-cannot-return-an-lvalue-container.md)) | [resolved](../news/2026-08/snitch-method-unimplemented.md) |
...
| `Language/traps.rakudoc:212` | `$.attr *= 2` inside a method throws `X::Assignment::RO` where current raku silently no-ops | [dollar-dot-attr-compound-assign-spurious-ro-error.md](../todo/deep/dollar-dot-attr-compound-assign-spurious-ro-error.md) |
...
| `Type/IO/Handle.rakudoc:959,1013` | ~~custom `IO::Handle` subclasses overriding `WRITE`/`READ`/`EOF` are never dispatched to by native print/say/read~~ | [resolved](../news/2026-09/custom-io-handle-routing-reaches-every-dispatch-entry.md) — both worked examples run, the `READ` pushback included ([2](../news/2026-09/custom-io-handle-read-pushback.md)) |
...
| file:line | one-line summary | ticket |
|---|---|---|
| `Language/grammars.rakudoc:112,132` | ~~`[+]`/`[-]` reduce meta-op silently gives `0` for Match/user-Numeric objects (arith ops bypass the numeric-coercion bridge `.reduce()`/binary `+` use)~~ | [resolved](../news/2026-08/reduce-metaop-numeric-coercion-bypassed.md) |
...
| `Type/DateTime.rakudoc:281,302` | ~~`.julian-date`/`.modified-julian-date` return `Num` (float noise) instead of an exact `Rat`~~ | [resolved](../news/2026-08/datetime-julian-date-returns-num-not-rat.md) |
...
| `Language/operators.rakudoc:1675` | ~~`[∘]` (empty-operand function-composition reduce) doesn't produce an identity `Callable`~~ | [resolved](../news/2026-08/compose-reduce-empty-list-not-identity-callable.md) |
| `Type/X/Phaser/PrePost.rakudoc:15` | ~~`X::Phaser::PrePost`'s message drops the failed `PRE`/`POST` condition's source text~~ (the condition was being deparsed from the AST; raku quotes the verbatim source slice) | [resolved](../news/2026-08/phaser-pre-post-message-drops-condition-source-text.md) |
...
| `Type/MixHash.rakudoc:99` | ~~`MixHash (^) MixHash` / `MixHash (+) MixHash` give garbage output (raw un-combined pairs) where the same ops on plain `Mix` work correctly~~ (real cause: the capitalised `Mix(...)`/`Bag(...)`/`Set(...)` *coercion* spellings shared the lowercase `.new` builders, so the operands were malformed) | [resolved](../news/2026-08/mixhash-set-operators-give-wrong-uncoerced-output.md) |
...
| `Type/Sub.rakudoc:19` | ~~a user `sub Int(...)` wrongly shadows the built-in `Int(...)` type-coercion call syntax (only `&Int(...)` should reach it)~~ | [resolved](../news/2026-08/user-sub-named-int-shadows-builtin-coercion-call.md) |
...
| `Language/numerics.rakudoc:353` | ~~calling `.^name` on a `MAIN`-bound `IntStr` argument corrupts a later, unrelated `IntStr.new(...).^name` (reports `Str`)~~ (no corruption: `MAIN` runs after the mainline, and its args were never `val()`-processed) | [resolved](../news/2026-08/main-allomorph-arg-name-corrupts-later-intstr-new.md) |
...
| `Type/Pair.rakudoc:61` | `.raku` on a `Hash` populated via slurpy `*%h` named-arg binding doesn't abbreviate `Bool::True`-valued pairs to `:key` the way real raku does (2026-08-26: re-measured — the signal is per-value Scalar-containerness of the hash's values, so this needs hash element containerization, not a `.raku` tweak) | [slurpy-hash-named-arg-raku-boolean-shorthand-missing.md](../todo/deep/slurpy-hash-named-arg-raku-boolean-shorthand-missing.md) |
...
| file:line | one-line summary | ticket |
|---|---|---|
| `Language/js-nutshell.rakudoc:384` | a user `multi prefix:<++>($a)` wins over the builtin `++` for `Int`/`Bool`/`Num` (the types raku's builtin has a *typed* candidate for); `is default` is not involved and the decision is made at parse time | [user-prefix-op-candidate-beats-builtin-typed-candidate.md](../todo/deep/user-prefix-op-candidate-beats-builtin-typed-candidate.md) |
...
| `Language/exceptions.rakudoc:428` | ~~`.resume` on a `die`-based exception raised inside a nested sub does not resume execution at the `die`'s call site~~ (the handler now runs at the throw point, ADR-0072) | [resolved](../news/2026-09/resume-returns-to-the-die-call-site-in-a-nested-sub.md) |
...
| `Type/Real.rakudoc:31` | ~~adding two custom `Real`-subclass instances (via `.Bridge`) produces an exact `Rat` where raku produces an approximate `Num`~~ | [resolved](../news/2026-08/real-subclass-generic-plus-produces-exact-rat-not-num.md) |
...
| Language/mop.rakudoc | 1 | 0 | 0 |
| Language/nativetypes.rakudoc | 1 | 0 | 0 |
| Language/syntax.rakudoc | 1 | 0 | 2 |
| Language/typesystem.rakudoc | 1 | 0 | 2 |
| Language/variables.rakudoc | 1 | 0 | 5 |
| Type/Attribute.rakudoc | 1 | 0 | 1 |
| Type/Backtrace.rakudoc | 1 | 0 | 2 |
| Type/BagHash.rakudoc | 1 | 0 | 3 |
| Type/Baggy.rakudoc | 1 | 0 | 3 |

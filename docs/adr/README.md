...
| [0109](0109-native-is-rw-scalar-parameter-reaches-the-light-call-fast-paths.md) | A native `is rw` scalar parameter reaches the light-call fast paths by reusing the existing `WrapVarRef`/`ContainerRef` transport, restricted to plain-lexical arguments | Proposed |

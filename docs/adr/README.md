...
| [0121](0121-instance-attributes-live-in-per-class-slots.md) | Instance attributes live in per-class slots, and each access site (`$!x`, accessors, `nqp::getattr`/`bindattr`) resolves its slot once through a layout-keyed inline cache | Proposed |

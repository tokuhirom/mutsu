...
- **Status**: Proposed（議論記録 — レベル選択・収集方式は最終承認待ち）
- **Date**: 2026-06-27
...
6. **層分割（Value 表現リワークの内訳）**：
   ...
   | **3b** | NaN-boxing（Value 48→8B） | GC 後でも可。型境界が 3a の型フィルタと一致 |
   | **3c** | biased reference counting（所有スレッドからの操作は非 atomic） | GC 後の独立 perf 課題 |
...
## 4. Open questions（未決 — 最終承認待ち）

### 4.1 ★ レベル1 か レベル2 か（最大の分岐）

「許容する書き直しの規模」をどこに置くか：

- **レベル1：`Arc → Gc<T>` ラッパ置換**（コンテナ系 79 箇所＋read 経路）
  → cycle collector 案。Rust 所有権を維持・最小侵襲。性能は refcount コストが残るが型フィルタで
  hot path は守る。**Claude の推奨。** 現在地（roast 1285・Phase A で追いつき中・Rust 所有+Arc 設計）
  からの費用対効果が圧倒的に高い。

- **レベル2：VM を "GC 前提" に全面再設計**（全 Value をハンドル/ヒープ管理、Rust スタックに生 Value を
  置かない、safepoint 導入）
  → MoarVM 式の precise moving tracing GC が視野に入る。**性能は最も伸びる**（bump alloc・refcost ゼロ）が、
  事実上インタプリタの再設計で、Rust で書く安全性の旨味を相当捨てる。年単位。mutsu の延長というより
  別物に近い。長期的に「MoarVM に性能で並ぶ独自実装」を本気で狙う場合のみ。

> 暫定方針：レベル1 で進める前提で Phase A を遂行。レベル2 は「JIT（層4）と合わせて再評価する長期
> オプション」として保留（JIT もオブジェクトレイアウト固定を求めるため、その時点で再検討の価値がある）。
...
> 別作業として実施する（このセッションでは「まず議論を詰める」を選択したため未反映）。

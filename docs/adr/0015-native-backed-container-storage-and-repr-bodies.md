...
- **Status**: Accepted (2026-07-27 — tokuhirom: *make `DBIish` work even if it means changing mutsu's
  ...
  **Progress (2026-08-19): P0, P1 (CStruct bodies), P2 (native-backed `Buf`/`Blob`, which took
  `DBIish` to raku parity on all nine files), P3a (native-backed `CArray[T]`, which put
  `NativeHelpers::Blob` on the battery gate) and P3b (native-backed `array[T]`) have all landed;

...
| **P4** — operator runbook | **done** — `.github/workflows/ecosystem-sweep.yml` is the nightly/dispatch path (D9's amendments) and docs/ecosystem-parity.md §8 documents both it and the local one. The local `make`-level wrapper P4 also listed was **dropped** (user decision, 2026-09-11): it would alias a documented command, and a Makefile line naming `scripts/ecosystem-sweep.py` fails `scripts/ci-docs-only.sh --check-inputs` (verified), whose only escape is taking the sweep scripts off the documentation allowlist and putting five build jobs back on every data PR |
...
The targeted Red remeasurement on 2026-09-15 (mutsu `63fe9c1f5`, Rakudo

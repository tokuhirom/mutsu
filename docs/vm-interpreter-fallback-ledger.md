...
| ~~`def_is_otf_compilable` の default 除外~~ | ~~default param 値を持つ multi/proto/single 候補~~ | — | **✅消化 (§D, #TBD)**: `def_is_otf_compilable` から `default.is_none()` を撤去。default は compiled binding の `bind_function_args_values` が interpreter と同一手順で評価（literal/earlier-param/method-call/閉包変数/nested-scope lexical 全て raku 一致）。nextsame は元引数を渡すので deferred 候補 default はその候補の default に解決（raku 一致）。`&g` Callable 経由 escaped-closure default の `Nil` は変更前から存在（非回帰・別軸）。fallback 85.7%→0%。pin `t/multi-default-otf-dispatch.t`(16) |

...
2. **Slice 2b — 配列要素 / ハッシュ値代入 [NEXT — 偵察完了 2026-06-18, 詳細 handoff=§10]**:
   `@aoa[i] = @row` / `%h<k> = @row` を cell 共有化
   (`assign_element_slot`/`hash_insert_through`)。AoA・HoA の参照共有。**未着手**(probe で
   mutsu `[1 2]` vs raku `[1 2 9]` を確認)。**BLOCKER 発見(2026-06-18 6th)**: 単一要素 array store
   (`vm_var_assign_ops.rs` の `exec_index_assign_expr_named_op_inner` ~3836 行)は
   `else if let Value::ContainerRef(cell) = &arr[i] { *cell.lock() = val }` で **ContainerRef 要素を
   再代入時に無条件 write-through** する。share-cell は再代入で REPLACE すべき(raku:
   `@aoa[0]=@row; @aoa[0]=(7,7)` は @row 不変)が、bind-cell は write-through。∴ 2b は per-element の
   share-vs-bind 区別(bound-index か専用 share マーカーのときだけ write-through)が要る。「値側で
   ContainerRef を push して通常 store に任せる」ショートカットはこの write-through で破綻する。
   **(§10 補足: probe で `@aoa[0]:=@row; @aoa[0]=5` も `=(7,7)` も raku は immutable-assign エラー
   =write-through を期待する raku 経路は存在しない→課題は「既存 `element-bind-cell.t` の write-through
   依存を壊さず `=`-share を replace 化する」点。read-leak は要素 cell では枯れ済み=2a より低リスク。)**
   - **[chained `$r = $q` DONE 2026-06-18, open-q#3 / #3267]**: scalar source の chained 共有は実装済み。
     Slice 2a の `WrapVarRef`+`MarkArrayShareContext` を **`MarkArrayShareSource(name_idx)` +
     `array_share_source` field** に統一リファクタ(WrapVarRef は plain `$x=$y` を bind 化する副作用が
     あったため不可)。compiler `try_emit_array_share` が `Expr::Var(n)` RHS も対象に追加。runtime は
     **source 値が deref して Array/Hash のときだけ共有**(`with_deref` ゲート)→ plain `$x=$y`(scalar
     source)・`:=` bound Int scalar の chained `=` は copy 維持。pin 計 33。make test/roast green。
3. **Slice 2c — bug ② の deref bind [array 形 DONE 2026-06-18(6th) / PR #3268]**: `my @a := @$n`(value-alias
   ...
   - **残**: ① **hash 形 `my %m := %$h`**: parser が `%$h` を `$h.hash`(MethodCall)に desugar するため
     `ArrayVar` のような simple-var bind 経路に乗らない(別経路)。② **sub-param 経由(headline bug②
     `sub f($n){ my @a := @$n; @a.push }`)**: param `$n` は `.push` 直接(case A)は伝播するが、その上に
     deref bind を重ねると伝播しない=param が `"n"` env key に `ContainerRef` を持たない(`arg_sources`
     return-writeback 共有=別機構)。Slice 2d の param-by-reference 化が前提。

...
4. **Slice 2d — sub 引数 / 戻り値の scalar-array 共有 [調査済み・Slice F 壁と確定 2026-06-19]**: probe では
   `g($x){$x.push}` / `return @arr` / `our $g = @z` の単純 push 伝播は **既に動作**(case A)。残は 2c-③
   の多段(`sub f($n){ my @a := @$n; @a.push }`)。**第24セッションで詳細調査し、call-boundary by-reference
   が必須=Slice F の壁と確定**:
   - **step 1(sub 内 aliasing)は実装可能**: param `$n` は **env でなく local slot のみ**に plain Array を持つ
     (`positional_light` が `unwrap_varref_value` で varref を剥がし plain 値を slot に格納)。slot を
     `ContainerRef` cell に昇格すれば `@a := @$n` がその cell を alias し、sub 内で `$n` も mutation を観測する。
   - **step 2(caller `@y` への伝播)が壁**: case A(`$n.push`)の伝播機構は **ArrayPush opcode でも
     `positional_light` の return writeback でもない**(両者とも param を *local* 扱いで skip)。`$n.push` は
     **CallMethodMut 経路**(`vm_call_method_mut_ops.rs:1558` `env_root_descended_mut(target_name)`)で env の
     コンテナを in-place 変異する別機構。deref-bind 経由で `$n` の slot を fresh cell に wrap すると caller `@y`
     との Arc 共有が COW detach で切れ、伝播しない。
   - **∴ 正しい 2d = call-site で「array/hash arg → scalar param」を共有 cell へ昇格**(hot param-binding 経路・
     高 blast-radius)。env↔locals コンテナ Arc 共有=first-class container/Slice F の前提作業に収束する。
     step-1 単独は read-leak(open-q#1)リスク+raku 非一致で ship 不可。なお旧記述の「array arg を

...
**2026-07-11 refinement（実装済み）**: candidate buffer は node への **`Weak` handle**
（`ErasedWeakGc`）を保持する。strong `Arc` clone を保持すると、buffered な node が
refcount 死したときに次の collect まで allocation 丸ごと（子の `Value` ツリー含む）が
生き延び、acyclic ゴミ大量発生 workload（grammar parse の `Match` ツリー等）で
RSS が数倍に膨らむ（`docs/grammar-parse-gc-churn.md`）。weak 保持なら:

- refcount 死は `MUTSU_GC=off` と同じくその場で解放（dead sweep がほぼ消滅）
- 真の cycle ゴミは自分自身を生かしているので drain 時の upgrade が必ず成功し、
  trial deletion の健全性は不変
- upgrade に失敗した dead entry は skip するだけ（header は node と共に消えているので
  触らない — 触れないことが正しさの条件でもある）
...
> **Update（2026-07-05・ADR-0003 受け入れ後）**: この節の「default off」は初期導入時の方針。
> ADR-0003 の受け入れゲート通過をもって **production 既定は `MUTSU_GC=on`** に切り替えた
> （トリガ = candidate バッファサイズ閾値 + adaptive backoff）。`MUTSU_GC=off` が明示 opt-out。
> 例外: crate 自身の unit-test ビルド（`cfg!(test)`）のみ既定 off — `cargo test` は並列スレッドで
> プロセスグローバルな collector 状態を共有し、in-process の safepoint collect がテスト間で
> 干渉するため（`gc::test_support` 参照）。GC-on の unit test は CI gc-stress ジョブが
> `MUTSU_GC=on` 明示 + single-thread で担保する。

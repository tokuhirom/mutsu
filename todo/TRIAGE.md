...
| [ADR-0053](../docs/adr/0053-do-whenever-produces-a-tap-on-the-stack.md) `do whenever` produces a Tap | `Accepted`, D1-D4 implemented (2026-09-07). `Tap.close` is now ordered against the emit via a shared global event sequence, so values emitted before the close still arrive; the twelve-row control table agrees with rakudo. | Closed out to `news/2026-09/whenever-tap-close-is-ordered-against-the-emit.md`. Residue: one dead-code question (§8). |

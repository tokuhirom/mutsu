...
| [interpreter-call-path-in-hot-loops](deep/interpreter-call-path-in-hot-loops.md) | Mostly resolved (13.8x → ~2x for the isolated hot-loop shape) | The file's own conclusion redirects further work to `eval-block-value-recompiles-every-call.md`, which was **retired 2026-08-20** (`news/2026-08/eval-block-value-recompiles-every-call.md`) — don't re-measure the already-closed shape, go to the `nested-sub-in-block-otf-recompiles-per-call` row below instead. |

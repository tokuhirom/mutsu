...
| General application logging (debug/info/warn/error) | `Log::Async` v0.0.17 (`zef:bduggan`) + `Terminal::ANSI` | **Selected, not yet bundled** | Artistic-1.0-Perl / MIT | Won the field (17 dependents, 3x the next candidate; most active maintenance, released 2026-03-09) over `LogP6` (richer feature set but 3 dependents, no commits since 2021), `Log::Dispatch` (0 dependents/stars), `Log::Syslog::Native` (syslog-only, out of scope), and `Log::Any` (abandoned since 2019); `Log::Simple`/`Log::Colored`/`Log::JSON` ruled out on AGPL/LGPL license. Distinct slot from the already-bundled `Log::Timeline` (task/event timeline instrumentation, not line-oriented logging). Blocked on one general parser gap in its `Terminal::ANSI` dependency, `todo/tickets/unit-monitor-declarator-not-supported.md` (file-scope `unit monitor Foo;` form of a user-registered `EXPORTHOW::DECLARE` keyword) | [logging.md](docs/batteries/logging.md) |
| XML parsing/generation | `XML` v0.3.6 (`zef:raku-community-modules`) | **Selected, not yet bundled** | Artistic-2.0 | Won the field over `LibXML` (a `libxml2` NativeCall binding) on ecosystem standing (45 dependents vs. 7 — the highest of any battery survey to date), zero runtime dependencies, and avoiding a hard system-library dependency (see the record's note on `libxml2`'s own 2025 maintainer-sustainability history). Both candidates are fully healthy under `raku`; `XML` is currently 1/15 under mutsu, blocked on two general bugs: `todo/tickets/grammar-token-param-dynvar-not-visible-in-subrule.md` (a grammar token's own dynamic-variable parameter default not visible inside a subrule it calls) and `todo/tickets/indirect-type-param-parse-failure-silently-drops-role-method.md` (an indirect type-name parameter constraint silently drops a role method instead of erroring). Vendoring follow-up tracked in `todo/tickets/bundle-xml-battery.md` | [xml.md](docs/batteries/xml.md) |

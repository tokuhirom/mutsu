...
| Monitors (`monitor` declarator) | native (upstream contract: `OO::Monitors` v1.1.7) | Native | — | **Working** — `use OO::Monitors` enables the `monitor` declarator; per-instance reentrant serialization at the compiled-method chokepoints. All 5 upstream test files (9 subtests, incl. the 4-thread increment race) pass, matching raku. A hard dependency of Cro::HTTP | [oo-monitors.md](docs/batteries/oo-monitors.md) |

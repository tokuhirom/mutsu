...
| Template engine | `Template::Mustache` (`zef:raku-community-modules`) | Adopted | Artistic-2.0 | **Working** — vendored + zero-config `use`; all 13 upstream files pass, including both official mustache spec suites. Chosen over `Template6` / `Jinja2` / 7 others by a measured survey | [templates.md](docs/batteries/templates.md) |
...
| JSON | `JSON::Fast` v0.20.1 (`timo/json_fast`) | Adopted | Artistic-2.0 | **Working** — vendored + zero-config `use`; all 14 upstream test files (931 assertions) pass against the bundled copy. The ecosystem's default JSON module: 541 of the 1,625 corpus distributions depend on it, against `JSON::Tiny`'s 32, and three bundled batteries `use` it. The native provider that used to answer this name is **deleted** (#8226) — the recorded "~50 missing `nqp::` ops" blocker was never measured: 42 of its 51 ops already worked | [json-fast.md](docs/batteries/json-fast.md) |
| JSON (reference) | `JSON::Tiny` v1.0 (`moritz/json`) | Adopted | Artistic-2.0 | **Working** — `use JSON::Tiny` resolves through the ordinary ladder and runs the bundled module's own source (the name-keyed interception was retired in #8183). All 6 upstream test files pass whole | [json-tiny.md](docs/batteries/json-tiny.md) |
| Monitors (`monitor` declarator) | `OO::Monitors` v1.1.7 | Bundled (verbatim) | `modules/OO-Monitors/` | **Working** — the upstream module runs verbatim: its EXPORTHOW::DECLARE block registers the `monitor` declarator, and MetamodelX::MonitorHOW drives registration through the HOW protocol (new_type/add_method/compose + method wraps). All 5 upstream test files (9 subtests, incl. the 4-thread increment race) pass, matching raku. A hard dependency of Cro::HTTP. (Was briefly provided natively, #5640; retired by the EXPORTHOW::DECLARE campaign) | [oo-monitors.md](docs/batteries/oo-monitors.md) |
| Conditional module loading (`use Foo:if(...)`) | `if` v0.1.5 (`zef:raku-community-modules`) | Bundled (verbatim) | Artistic-2.0 | **Working** — the upstream module runs verbatim (ADR-0098): mutsu answers `Raku.legacy` with `False`, so the pragma takes its `$*LANG.define_slang` branch, and the *actions* role it mixes in registers its one overridden production (`statement-control:sym<use>`) as a parser mode. Nothing in the upstream suite is whitelisted yet — its one file counts loads through a caller dynamic variable, which a module load does not write back on mutsu ([#8229](https://github.com/tokuhirom/mutsu/issues/8229)) — but the pragma itself is pinned by `t/modules/import-export/use-if-pragma.t`, 9/9 under `RAKUDO_RAKUAST=1` too. A hard dependency of `Crypt::Random`, and hence of Cro; 18 corpus distributions depend on it. (Was provided natively until 2026-09-13) | [if-pragma.md](docs/batteries/if-pragma.md) |
...
| HTTP/2 header compression (HPACK) | `HTTP::HPACK` (`zef:raku-community-modules`) | Adopted | Artistic-2.0 | **Working** — vendored + zero-config `use`; both upstream test files (57 subtests: the RFC 7541 appendix-C examples, Huffman coding, dynamic-table eviction) pass against the bundled copy, matching raku. A hard dependency of Cro::HTTP (HTTP/2) | [http-hpack.md](docs/batteries/http-hpack.md) |
| Message digests (MD5 / SHA / RIPEMD / HMAC) | `Digest` (`zef:grondilu`) | Adopted | Artistic-2.0 | **Working** — vendored + zero-config `use`; MD5, SHA-1, all four SHA-2 widths, all four SHA-3 widths plus SHAKE, RIPEMD-160 and a generic HMAC, in pure Raku with zero dependencies and no native code. 3 of 4 upstream test files pass and are gated (`md5.t`, `sha.t`, the RFC 4231 HMAC vectors); `ripemd.t` is correct but exceeds the gate's per-file budget on its 1 MB vector. Chosen over `Digest::SHA1::Native` / `OpenSSL::Digest` / 4 others by a measured survey | [digest.md](docs/batteries/digest.md) |
| Keyed-hash message authentication (HMAC) | `Digest::HMAC` (`zef:jjmerelo`) | Adopted | MIT | **Working** — vendored + zero-config `use`; both upstream test files (9 subtests: the Wikipedia MD5/SHA-1/SHA-256 vectors and the short/exact/long key block-size cases) pass against the bundled copy, matching raku. Generic HMAC over any hash routine (`&sha256` from `Digest` or `OpenSSL::Digest`); a hard dependency of `JSON::JWT`, and therefore of Cro's WebToken auth. Distinct from the `Digest` distribution's own named-argument `HMAC` module above | [digest-hmac.md](docs/batteries/digest-hmac.md) |
| JSON Web Tokens | `JSON::JWT` v1.1.2 (`zef:raku-community-modules`) | Adopted | MIT | **Working** — vendored + zero-config `use`; both upstream test files pass against the bundled copy, matching raku. A hard dependency of Cro::HTTP's `Cro::HTTP::Auth::WebToken` (bearer-token auth) | [cro-deps.md](docs/batteries/cro-deps.md) |
| CBOR codec | `CBOR::Simple` v0.1.4 (`zef:japhb`) | Adopted | Artistic-2.0 | **Sufficient for Cro** — 1/7 upstream files pass; `Log::Timeline`'s narrow CBOR-output usage works, but the module's own broader suite (parameterized `array[...]` types, diagnostic notation) has real gaps — see the record | [cro-deps.md](docs/batteries/cro-deps.md) |
| Half/bfloat16 float codec | `TinyFloats` v0.0.5 (`zef:japhb`) | Adopted | Artistic-2.0 | **Working** — vendored + zero-config `use`; all 5 upstream test files pass against the bundled copy, matching raku. `CBOR::Simple`'s dependency (CBOR minor-type-7 float encodings) | [cro-deps.md](docs/batteries/cro-deps.md) |
| Async TLS sockets | `IO::Socket::Async::SSL` v0.8.2 (`zef:raku-community-modules`) | Adopted | Artistic-2.0 | **Sufficient for Cro** — `Cro::TLS`'s transport works (proven by Cro::HTTP's own 35/35 suite), but the module's own suite fails wherever it opens a real listening socket (`IO::Socket::Async::Listener` is missing `.Supply` in mutsu) — see the record | [cro-deps.md](docs/batteries/cro-deps.md) |
| Structured task/event logging | `Log::Timeline` v0.5.2 (`zef:raku-community-modules`) | Adopted | Artistic-2.0 | **Sufficient for Cro** — 1/5 upstream files pass; Cro's own timeline instrumentation usage works, but the module's own output-backend round-trip suite (CBOR/JSON-lines/socket) has real gaps — see the record | [cro-deps.md](docs/batteries/cro-deps.md) |
...
| CSV read/write | `Text::CSV` v0.022 (`zef:Tux`) | Adopted | Artistic-2.0 | **Working** — vendored + zero-config `use`; all 32 upstream test files (22,696 assertions) pass against the bundled copy (`99_meta.t` waived — `Test::META` dist-metadata QA, not CSV functionality). Chosen over `CSV::Table` on test depth and API fit (from-scratch generation with no pre-existing template file needed). Depends on the already-bundled `Slang::Tuxic`/`Slangify`/`File::Temp`/`File::Directory::Tree` — added no new dependency | [csv.md](docs/batteries/csv.md) |
...
| TOML config parser/writer | `Config::TOML` v0.1.3 + `Crane` v0.1.2 (both `zef:raku-community-modules`) | **Selected, not yet bundled** | Unlicense ×2 | Won the field (license, 0-net-dep chain, maintainer stewardship, richest upstream suite: 19 files) over `TOML` (7 dependents but requires raw `nqp::` op support — out of scope) and `TOML::Thumb` (single narrow blocker, runner-up). **Re-measured 2026-08-26: still 0/19** (`Crane` 3/15) against raku's 19/19 and 15/15. The original bare-identifier-adverb parser gap is fixed and the grammar now parses documents, but `Crane`'s descent still fails on array paths, `.add`/`.copy` deep-clone and `X::Crane::*` classification, so every `Config::TOML` file that builds a result fails. See the record's remaining-work list | [toml.md](docs/batteries/toml.md) |
| UUID generation | `UUID` (`github:retupmoca`) | Adopted | MIT | **Working** — vendored + zero-config `use`; the upstream test file passes against the bundled copy, matching raku. Chosen over `UUID::V4` / `LibUUID` on ecosystem standing (30 dependents vs. 0 / 3). Exposed a general dispatch-priority bug: a user class's own `has $.bytes` accessor was shadowed by the target-agnostic native `.bytes` builtin (missing from the Cool-only-builtin gate, ADR-0051 P4) — fixed for every class, not just this module | [uuid.md](docs/batteries/uuid.md) |
| General application logging (debug/info/warn/error) | `Log::Async` v0.0.17 (`zef:bduggan`) + `Terminal::ANSI` v0.0.25 | Adopted | Artistic-1.0-Perl / MIT | **Working** — vendored + zero-config `use`; `use Log::Async <trace>; trace 'x'` runs against the shipped binary. 11 of 17 upstream `Log::Async` files and 5 of 8 `Terminal::ANSI` files pass and are gated (up from 2/17 and 2/8 — the blocking `unit monitor Foo;` parser gap was fixed 2026-08-26); the remaining files are ordinary interpreter gaps listed in the record, none in the core log-a-message path. Won the field (17 dependents, 3x the next candidate; most active maintenance, released 2026-03-09) over `LogP6`, `Log::Dispatch`, `Log::Syslog::Native` and `Log::Any`; `Log::Simple`/`Log::Colored`/`Log::JSON` ruled out on AGPL/LGPL license. Distinct slot from the already-bundled `Log::Timeline` (task/event timeline instrumentation, not line-oriented logging) | [logging.md](docs/batteries/logging.md) |
| XML parsing/generation | `XML` v0.3.6 (`zef:raku-community-modules`) | **Adopted** (`modules/XML/`) | Artistic-2.0 | Won the field over `LibXML` (a `libxml2` NativeCall binding) on ecosystem standing (45 dependents vs. 7 — the highest of any battery survey to date), zero runtime dependencies, and avoiding a hard system-library dependency (see the record's note on `libxml2`'s own 2025 maintainer-sustainability history). Both candidates are fully healthy under `raku` (15/15 and 70/70). **Bundled 2026-09-02 at 15/15**, matching `raku` — up from 1/15 at survey time, every step a general interpreter fix (grammar dynamic-variable parameters, indirect type-name constraints, a backreference inside a `[...]` group, ADR-0061's lexical `$self`, slurpy junction binding and `IO::Path(Str)` coercion, a Capture inside a slipped container, and list stringification calling each element's `Str`). All 15 files are on `batteries-whitelist.txt`; `t/xml-battery.t` is the in-repo smoke test | [xml.md](docs/batteries/xml.md) |

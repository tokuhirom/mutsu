...
| JSON | native `to-json` / `from-json`, `JSON::Tiny` v1.0 (`moritz/json`) `Grammar`/`Actions` | Native + Adopted | Artistic-2.0 | **Working** — `use JSON::Fast` / `use JSON::Tiny` still resolve to the native fast path (>1000x faster than the real grammar on mutsu's regex engine — see the record); `JSON::Tiny::Grammar` / `::Actions` are not intercepted and now run the bundled real module. All 6 upstream `JSON::Tiny` test files (135 subtests) pass against the bundled copy | [json-tiny.md](docs/batteries/json-tiny.md) |

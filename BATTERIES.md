...
| JSON | `JSON::Tiny` v1.0 (`moritz/json`), plus a native `JSON::Fast` provider | Adopted + Native fallback | Artistic-2.0 | **Working** — `use JSON::Tiny` resolves through the ordinary ladder and runs the bundled module's own source (the name-keyed interception was retired in #8183). `JSON::Fast` is not vendored yet, so the native `to-json`/`from-json` answer it — but only after the ladder comes up empty, so an `-I`/`MUTSULIB`/site-repo copy wins. The recorded "~50 missing `nqp::` ops" blocker was never measured and is gone: 42 of its 51 ops already worked, the nine that did not are implemented, and 13 of the 14 upstream test files now pass against the real distribution (#8226). 5 of 6 upstream `JSON::Tiny` test files pass whole; `01-parse.t` is 92/93 on a `use`-is-not-BEGIN-time gap (#8201) | [json-tiny.md](docs/batteries/json-tiny.md) |

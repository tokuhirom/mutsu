...
| XML parsing/generation | `XML` v0.3.6 (`zef:raku-community-modules`) | **Selected, not yet bundled** | Artistic-2.0 | Won the field over `LibXML` (a `libxml2` NativeCall binding) on ecosystem standing (45 dependents vs. 7 — the highest of any battery survey to date), zero runtime dependencies, and avoiding a hard system-library dependency (see the record's note on `libxml2`'s own 2025 maintainer-sustainability history). Both candidates are fully healthy under `raku` (15/15 and 70/70). **Re-measured 2026-08-26: `XML` is 5/15 under mutsu**, up from 1/15 at survey time — its two original blockers are both fixed, and a third (a backreference inside a `[...]` group never resolving, which broke every closing tag) was fixed with the re-measurement. The remaining gate is `todo/deep/lexical-self-collides-with-invocant.md`: `XML::Element.AT-POS` returns a `Proxy` whose `FETCH` closes over `my $self = self`, and mutsu stores a `$self` scalar under the same env key as the invocant, so the fetch recurses until the stack overflows (7 of the 10 failures). Vendoring follow-up tracked in `todo/tickets/bundle-xml-battery.md` | [xml.md](docs/batteries/xml.md) |

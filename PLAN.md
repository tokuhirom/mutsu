...
- [ ] **残（別根・フォローアップ）: ループ本体の body-local `my` が per-iteration scope にならない**。
      `for/while` 本体は `compile_body_with_implicit_try` でコンパイルされ、bare block（`Stmt::Block`→
      `BlockScope`）と違って body-local `my $x` を tear down しないため leak する（`for 1..3 { my $x = $_;
      @a.push({$x}) }` → `333`、raku `123`）。修正にはループ本体を BlockScope で包む必要があり、topic 復元/
      writeback との相互作用で blast radius が大きいので別タスク。
- [ ] **本丸: 自由変数を indexed upvalue（共有セル `ContainerRef`）として捕捉**し、closure が親 env を
      名前で読むのをやめる。これで兄弟クロージャの捕捉変数共有（同一 frame 由来の複数クロージャが
      `$x` を共有: `my ($g,$s)=make(); $s(42); $g()` が raku `42` / mutsu `1`）も根治する。B（scoped env）と連動。

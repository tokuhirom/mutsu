    ...
    - **全パス変換チェックリスト**（各スライスは `make test` + 関連 roast をローカル検証、全 roast は CI）:
      ...
      - [ ] **Slice 6.3（最後のフラグ）**: `env_dirty` → tree-walking interpreter が env を名前変更した後の slot 再 sync 用。
            ...
            **計測で確認した残コスト2点（2026-06-07、詳細は docs/vm-dual-store.md「Remaining method-call dual-store costs」）**:
            ...
            (2) **ネストしたメソッド呼び**（`method f(){ self.d() }`）ごとの O(env) flatten + deep copy
            （bench-class の `full-info`→`describe`、env_deep_copies が呼び数ぶん）。根因は scoped overlay が**1段**
            （`Env::scoped_child` が単一 parent_overlay しか取れず、scoped な caller の上にネストすると親 tier を失うため
            flatten で潰す）。**次の具体ステップ＝多段 overlay（overlay Arc のチェーン化）でネスト時に flatten しない**。
            第一級コンテナ移行（🟣）と地続きの大きめ変更。

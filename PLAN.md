...
- **§2 Phase B** は層3a（GC）・層3b（NaN-boxing）完了、層4 JIT は J1（骨組み #4471）・
  J2（Tier A 網羅 #4474）・J3（呼び出し規約/IC #4476）完了 —
  次スライスは J4（Tier B インライン + hot-loop entry・ADR-0004）。
...
- [ ] **層4 JIT（Cranelift・= §5 Lever 4）**: **J1 骨組み（#4471）・J2 Tier A 網羅（#4474）・
      J3 呼び出し規約/IC（#4476）完了（2026-07-12〜13・詳細 = news/2026-07.md）** —
      J3 で CallMethod/CallMethodMut 専用 shim・呼び出し規約の per-call alloc 除去
      （`to_map()` 全クローン廃止 = attr cell read-through 化）・`type_matches_value`
      tag/class fast accept・CallFunc name Symbol の const_sym memoize。実測（main と同一
      静音ウィンドウ A/B・スケール版ベンチ）: method-call の JIT on が J2 の -6% 回帰 →
      **+3%** に反転、bench-fib は interpreter +5.4% / JIT on **+12.5%**。roast 全
      whitelist 1384 ファイルが JIT on（threshold=2・release）で green。残フェーズ =
      **J4 Tier B インライン（NaN-box タグ分岐つき算術 CLIF 展開・hot-loop entry
      （backedge カウンタ）。int-arith 級 top-level/compound ループ・for-loop body
      （try_enter フックなし）・JIT→JIT の per-call bind インライン化・args Vec alloc
      除去はここの圏）** → J5 既定 on =
      [ADR-0004（Accepted 2026-07-06）](docs/adr/0004-jit-strategy.md)。
...
- **Lever 3: threaded dispatch — 凍結**（2026-07-06 ユーザー承認・[ADR-0004](docs/adr/0004-jit-strategy.md) §2.5 J0）:
      ...
      JIT が頓挫した場合のみ復活。
- [ ] **Lever 4: JIT（Cranelift）= ADR-0001 層4 — J1（#4471）・J2（#4474）・J3（#4476）完了、
      次 = J4 Tier B インライン** / Lever 5: 型制約チェックの tight-loop 省略（J3 の
      `type_matches_value` fast accept で大半を回収済み）。
      残フェーズ（J4 Tier B インライン → J5 既定 on）とゲートは

...
- [x] **`.new`（デフォルトコンストラクタ）デフォルト構築 fast path** を VM native 化
      (`simple_ctor_plan` + `try_native_default_construct`)。カスタム new/BUILD/TWEAK・属性デフォルト・
      ロール・型/where/smiley 制約・属性名重複なし、`$`属性のみの plain クラス（**継承含む**）を
      VM 内で構築。method-call.raku 20%→**0%**、bench-class 13.8%→**0%**（共にフォールバック皆無）。
      残り（属性デフォルト/BUILD/TWEAK/カスタム new あり）は引き続きフォールバック。
- [ ] **`.new` 残り**: 属性デフォルト（stored `Expr` の VM 評価が必要）、BUILD/TWEAK（コンパイル済み
      submethod ディスパッチ）、カスタム new。これらは「クラスレジストリ/属性デフォルト/型強制を
      VM 所有データへ移す」大物（§1.1）で後続。
...
- [ ] `EVAL` / symbolic deref / `CALLER::` — **tree-walk ではない**点に注意。`EVAL` の実行は
      すでに `eval_block_value` で **compile→バイトコード→サブ VM 実行**（`run_compiled_block` が
      `VM::new(interp)` する）。残る結合は (1) 入口が `Interpreter::call_function` 経由で
      フォールバック計測に乗る計測ノイズ、(2) 共有可変状態（env/classes/roles レジストリ）の
      オーナーが Interpreter 構造体である点。後者は **レバー B（状態オーナーシップ）に収斂**する
      課題であり、「恒久的に interpreter でよい例外」ではない。symbolic deref / `CALLER::` も
      reflective な共有 env アクセスが要るだけで tree-walk は不要。最終的には VM がネイティブに
      compile+run パイプラインへ到達できるようにして計測ノイズも消す。

...
- [ ] **Slice 6.3 残り（= `env_dirty` フラグ削除 = レバー B 完遂）**。フラグはまだ存在し、稀な
      ...
      - (A) **最優先=regex/smartmatch の pull**（bench-string `locals_pulls=5003`、1/match）。
        `src/vm/vm_comparison_ops.rs:~1261` の無条件 `env_dirty=true`。`~~` は `_`/`var_name`/`$/`/`$0`/`$<>` を
        by-name 書き込み＝**step2 reverse write-through の典型**（slot があれば `self.locals[slot]` も書いて
        blanket を外す。`$/`等は code.locals でない＝`could_name_caller_local` 流 skip 候補、`_`/var_name は slot 書き）。
      - (B) 残り step2: 他の by-name var/control op（vm_var_get_ops / vm_register_ops の `&sub`・型名 / vm_control_ops / vm_data_ops）。

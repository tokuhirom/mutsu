...
#### ✅ CP-3 完了（2026-06-15, PR #3102 + #3104）: Interpreter 撤去ゴール達成 — VM decoupling 完結

**🔴 最優先だった「tree-walking Interpreter 廃止 = VM decoupling」は CP-3 で完結した。** 旧来 `VM { interpreter: Interpreter, ... }`
の 2 struct 相互参照 + ping-pong（`mem::take(self)`+`VM::new(self)`+`*self=interp`）を撤去し、**単一 struct `Interpreter` が bytecode VM
そのもの**になった。終状態 = 「VM(=Interpreter) が唯一の実行エンジン・全状態 VM 所有・`env_dirty` dual-store のみ VM 内部最適化として残存」。
- **#3102（collapse 本体, CI フル `make roast`+`make test` PASS, net −794 行）**: survivor = **`Interpreter`**（公開エントリ型
  `Interpreter::new()`/`.run()` を main/lib/repl/test が使用 → エントリ・公開 API を無改修に保つため。doc の当初案「VM survivor」とは逆）。
...
- **#3104（cleanup）**: `type VM = Interpreter` alias を完全撤去（`impl VM`→`impl Interpreter`、`VM::`→`Interpreter::` 等）し
  コードベースから `VM` 型名を排除。dead な `Env::poisoned` ガード（field+`assert_not_poisoned`+6 caller）を除去。
- `env_dirty`/`ensure_locals_synced`/`sync_locals_from_env` dual-store は CP-2 通り**意図的に存続**（EVAL + slot-locals 共存下で
  構造的に必須・perf。Interpreter オブジェクトの有無とは無関係な VM 内部最適化）。

**→ 次の戦略作業へ**: VM decoupling 完結により、主作業は 🟣第2優先「第一級コンテナ」残バックログ / Track C 並行（共有セル）/
perf / Q3 module 互換・roast backlog へ移る（下記各セクション）。
...
#### CP-3: Interpreter オブジェクト撤去（= 最終ゴール本体）— ✅ 完了（#3102 + #3104, 上の「現在地」参照）

> **以下は着手前の分析・計画（記録として保持）。実行結果は上の「✅ CP-3 完了」節が正準。**

> ##### ★最重要の再フレーム（2026-06-15 実測確定）: 「tree-walk 実行エンジン」は既に存在しない
>
> CP-3 着手前にコードを実測した結論。**`Interpreter` はもはや tree-walker ではない**:
> - `eval_expr`/`eval_stmt` 等の **AST 評価器は存在しない**。`&Expr`/`&Stmt` を取る runtime/ の関数は
>   すべて**静的解析**（scan/collect/validate/rewrite）で実行ではない。
> - sub/block/method の実行は `run_compiled_block` = **compile → bytecode → VM**（ping-pong）。
>   `call_sub_value`/`eval_block_value`/`call_function`/`type_matches_value` も全て「コンパイルして VM で走らせる」
>   carrier であって tree-walk ではない（subset `where` も既に bytecode だった）。
> - `CompiledCode.stmt_pool` の `Stmt` は**宣言 AST**（class/sub/role/enum）で、registration が runtime に
>   **コンパイル**する用途。tree-walk 実行ではない。VM が実行時に `Expr` を評価する箇所も無い。
>
> → **「tree-walk interpreter を削除する」の正体 = `Interpreter` struct を VM へ溶かす（dissolve）こと**。
> 残る `Interpreter` = ①状態（73 フィールド）②carrier dispatch（compile→VM-run のラッパ）③native builtin/メソッド
> ④registration/MOP ⑤静的解析ヘルパ。**「tree-walk を bytecode に書き直す」重い仕事はもう無い**＝これは
> アルゴリズム的難所ではなく**構造的な作業（量 + ping-pong の結び目）**。原理的ブロッカーは無い（最ホットの env は
> CP-1 で移管済・carrier は loan で実証済）。
>
> ##### 完遂までの残量（実測 2026-06-15）と道筋
>
> | 指標 | 数 | 作業 |
> |---|---|---|
> | `Interpreter` フィールド | 73（env+5 ハンドル済、残 ~67 は大半 plain data） | VM へ fold |
> | VM→`self.interpreter.*` サイト | 718 | `self.X` へ書換 |
> | VM が呼ぶ distinct interpreter メソッド | 192 | `impl VM`/free 関数へ移設 |
> | `loan_env!` 経由（carrier 借用） | 415 | ping-pong 解消で整理 |
>
> 完遂の道筋: **(c) ping-pong 解消（`mem::take(self)`+`VM::new(self)` の双方向所有 → VM 上で直接再入）が唯一の
> 本物の設計仕事**。これが通れば (a) フィールド fold / (b) メソッド移設 / (d) サイト書換 / (e) struct 削除 は
> **量の問題（並列化可能・CI 安全網）**に落ちる。**∴ 着手は (c) の PoC から**（`run_reuse`/`run_range` で in-place
> 再入実行の素地は既にある＝ping-pong は carrier が Interpreter 側に在ることの産物）。**完遂しないと ROI ゼロ**
> （半マージは2つの綺麗な struct より悪い）なので、PoC で (c) が解けると確認できたら **single collapse campaign**
> として一気通貫で進める（perf スライスは打ち切り）。

設計 = [docs/vm-state-ownership.md](docs/vm-state-ownership.md)。`Interpreter` = **73 フィールド（状態）** +
**carrier dispatch（compile→VM-run。tree-walk ではない）**。VM からの汎用 dispatch フォールバックはベンチ ~0%（§1/§2 ほぼ枯渇）。
...
- [ ] **Track 2 — tree-walk carrier を VM-native 化（Role B・実行役割の除去・高難度）**:
      - [~] **subset `where` / 型制約**（`type_matches_value` ~690 行）→ where 式を bytecode コンパイルして sub-VM 実行。
            **重要な実測訂正**: where predicate は**既に bytecode 実行**だった（`eval_block_value`→`compiler.compile`）。
            ...
            raku 完全一致。**第2スライス DONE**: single-param pointy `where -> $x {...}`（= `Lambda{param}`、型/デフォルト/
            トレイト/sub-sig 付きは `AnonSubParams` へ落ちる＝`Lambda` は必ず untyped 単純パラメータ）も `env[param]` を bind
            して inline 化。named-param subset も 2.1x（24.5s→11.5s）。残: `type_matches_value` 全体の VM-native 化（subset arm
            以外も含む大物）・multi/typed param predicate（`AnonSubParams`）の inline 化。
...
**推奨着手順（再フレーム後）**: 上記のとおり tree-walk 実行は既に無く残りは構造的 collapse なので、**まず ping-pong
解消の PoC（(c)）** を行い「`Interpreter` を VM へ溶かしたとき carrier が VM 上で直接再入できるか」を実証する。通れば
single collapse campaign（(a) フィールド fold → (b) メソッド移設 → (d) サイト書換 → (e) struct 削除）へ。Track 1/Track 2 の
個別スライスはこの campaign に吸収される（独立 perf スライスは ROI に寄与しないので打ち切り）。

> ##### ★次セッションの計画（2026-06-15 確定）— incremental は枯渇、残りは big-bang collapse 本体
>
> 2026-06-15 のセッションで **PoC（(c)）= `VM::run_nested` を実証し（#3095）、clean carrier 2 本（run_block_raw /
> eval_block_value pure-expr）を変換**。その後 `MUTSU_VM_STATS` 計測 + 精読で **「拾える incremental スライスは尽きた」**
> ことを確定した:
> - dispatch fallback = drained（残 `new`=ctor〔③〕/ `isa`/`does`=既に VM-native / MOP / EVAL=carrier・単桁 diffuse）。
> - hot state フィールド（env/readonly_vars/let_saves/var_defaults…）は per-call の interpreter dispatch state で
>   （`save_readonly_vars`/`restore_readonly_vars` が 9 ファイル）、`&mut`-return accessor も絡み **早期 fold 不可**。
> - 循環: hot field は interpreter dispatch carrier が save/restore → carrier 除去なしに field 除去不可 → carrier は
>   既に VM native（`call_compiled_closure`）の fallback → fallback 除去＝struct 除去。**∴ collapse は一括（big-bang）。**
>
> **→ 次セッション = `Interpreter` struct の big-bang collapse 本体に着手する。** 詳細な実行計画（手順 1〜5・リスク・
> resourcing・着手前チェックリスト）は **[docs/vm-state-ownership.md](docs/vm-state-ownership.md)「CP-3 final collapse — 次セッション
> 実行計画」節**が正準。要点: long-lived branch `cp3-collapse` で ①struct マージ + 718 サイト `self.interpreter.X`→`self.X`
> 書換（機械的・multi-agent fan-out 可）→ ②~192 carrier/dispatch メソッドを `impl VM` へ移設 + ping-pong を `run_nested`
> 直接再入へ（**各サイトで env→locals 再同期を明示**＝run_nested PoC の最重要教訓・手動検証必須）→ ③loan/ping-pong 機構撤去
> → ④struct 削除。dual-store（`env_dirty`）は永続で残す。**incremental perf/carrier スライスは足さない**（完遂前 ROI ゼロ）。

...
- [ ] **method-call ホットパス整理（計測駆動）= method-call <2x 継続**:
      ...
      - [x] mro_readonly キャッシュ（#3859・**最大の単独 win**）: `mro_readonly`(&self) が BFS で毎回 ancestor を
            ...
            （BFS 結果と証明的に等価）。
      - [ ] **次: symbol intern round-trip の残り**: `cn = class_sym.resolve()` の String 確保が downstream の `&str`
            dispatch API のため残る。`&str` dispatch API を Symbol 化すれば除去（`Symbol::intern`+`memcmp` がまだ ~14%・
            大型リファクタ）。
      - [ ] `Value` clone/drop（bench-class の ~50%）＋ instance 構築 HashMap ＋ `merge_method_env` は Lever 2
            （NaN-boxing・GC 後）待ち。
...
2. [ ] **ローカル完結コマンドの仕上げ**: `--help`/`info`/`locate` 等を完全動作。`where .so` no-args 誤マッチは
   **修正済み（#3866）**＝named param の `where` 制約を**省略時にも default/type-object 値に対して評価**するよう
   `binding_signature`（バインド経路）と `args_matching`（候補選択経路）の両方を直した。`multi sub MAIN(Bool
   :version($) where .so)` は引数無しで発火しなくなった（topic が `Bool` type object・`.so`=False で候補棄却）。
   残ギャップ＝**リテラル位置引数 + slurpy の MAIN ディスパッチ**（`multi MAIN('info', *@ids)` / `multi MAIN('locate',
   Str $p)`）。mutsu は第1引数のリテラル値を無視して常に同一候補へ誤マッチし slurpy も空になる。なお raku は
   `info Foo Bar` / `locate /x/y` を**いずれも Usage で棄却**する（リテラル+追加 positional の組み合わせを許さない・要仕様確認）。

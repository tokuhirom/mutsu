...
- [ ] **Slice 6.3 残り（= `env_dirty` フラグ削除 = レバー B 完遂）**。フラグはまだ存在し、稀な
      ...
      - (C) **残り step2（次の着手）**: 残 setter は3種に分かれる（当初「by-name var/control op」と一括りにしていたが非均一）:
        ①別 dispatch 経路の post-call 保守マーク（vm_var_get_ops の bareword-term、vm_register_ops の trait/`call_function`）—
        `call_compiled_function_named` に純粋性シグナル（method の `method_dispatch_pure` 相当）を足して精密化。
        ②genuine by-name write の clean drop / reverse-write-through（vm_data_ops:418 は push が既に slot 復元済み＝冗長、
        vm_register_ops:824/908 は `locals_set_by_name` 済み＝冗長、vm_var_assign_ops:4018/4042 は `:=` 要素 SlotRef）。安全だが cold。

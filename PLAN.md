...
- [ ] **残（延期・大）: 非ループの一般コンテナ捕捉**（`my ($g,$s)=make(); $s(42); $g()` → raku `42`、
      ...
      の非ループ兄弟 2 ケースは `todo`。
- [x] **Slice 3: body-local `my` の env leak/clobber（クロージャ非依存の正しさバグ）**。`my $x=99; for/while/loop/
      repeat/if { my $x=... }; say $x` が外側 `$x` を**クロバー**して最終値（raku は `99`）になる正しさバグを
      根治。dual-store の env が name-keyed で、ループ本体が全 local を `needs_env_sync` 化するため、内側 `my $x`
      ...
      `if`/`else` は実行1回なので既存の BlockScope（完全 env+locals save/restore）でラップ（`body_declares_block_local`）。
      pin `t/block-local-my-scope.t`（15件）。**残**: 外側束縛が無い純粋 leak（`for {my $x=...}; say $x`）の

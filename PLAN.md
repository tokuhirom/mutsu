...
- [ ] **upvalue 化 Slice 2（フォローアップ）: 共有 `ContainerRef` セルで intra-iteration mutation を正しく**。
      Slice 1 は捕捉時に**値を凍結**するため、捕捉**後**の同一反復内変異が見えない
      （`for ... { my $x=1; my $c={$x}; $x=2; ... $c() }` は raku `2`、mutsu `1`）。本来クロージャは
      バインディング（コンテナ）を捕捉する。box-on-capture で `owned_captures` を `ContainerRef` 共有セルにし、
      反復ごとに fresh セルを作れば intra-iteration 共有も満たせる。`my` が反復ごとに fresh セルを作る
      （= per-iteration teardown）必要があり、`sync_env_from_locals` の slot 結合とも関わる。
- [ ] **本丸の残: 兄弟クロージャの捕捉変数共有**（同一 frame 由来の複数クロージャが `$x` を共有:
      `my ($g,$s)=make(); $s(42); $g()` が raku `42` / mutsu `1`）。`closure_captured_state` を closure-id
      単位でなく**バインディング単位**（`ContainerRef`）にすれば根治。Slice 2 の ContainerRef 基盤と共通。
- [ ] **残（別根）: body-local `my` の env leak**（`for 1..3 { my $x=$_ }; say $x` が raku 未宣言エラー、
      mutsu は最終値）。クロージャ捕捉は Slice 1 で免疫化済み（owned は caller env を見ない）だが、
      非クロージャの post-loop 名前読みは依然リークを拾う。ループ本体の per-iteration scope tear down が必要
      （slot 再注入と絡むので Slice 2 の ContainerRef/teardown と併せて）。

...
- [ ] **Slice 6.3 残り（= `env_dirty` フラグ削除 = レバー B 完遂）**。フラグはまだ存在し、稀な
      ...
      - (C'') **②native `@a.push` fast path — 完了**（実は cold でなく hot: `@a.push($_)` in loop は頻出）。
        vm_data_ops の push fast path は target 配列のみを env で変異させ、結果を直前で local slot へ復元済み
        （reverse-write-through）なので blanket マークは冗長。撤去で `for ^5000 { @a.push($_) }` の locals_pulls 5001→2。
        interpreter-fallback push 枝（shared/shaped/非単純配列）は保守的に維持。pin `t/push-env-dirty.t`（13）。
      - (C''') **残り step2（真に cold・zero-benefit、env_dirty 削除を一括でやる時のみ）**: vm_register_ops の
        declaration-time trait handler（824/908/932/1079/1095＝`is default`/`is Buf`/typed/subset/enum、宣言毎1回）、

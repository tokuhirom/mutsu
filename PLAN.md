...
- **§2 Phase B** は層3a（GC）・層3b（NaN-boxing）完了、層4 JIT は J1（骨組み #4471）完了 —
  現行スライスは J2（Tier A opcode 網羅 + jit-stress CI・ADR-0004）。
...
| **C. JIT** | 独自メリット（**いまここ** — J1 骨組み完了 #4471・次は J2） | §5 Lever 4 |
...
- [ ] **層4 JIT（Cranelift・= §5 Lever 4）**: **J1 骨組み完了（2026-07-12 #4471・詳細 =
      news/2026-07.md）** — `MUTSU_JIT=on`（feature `jit`・既定 off）で hot chunk が native 実行・
      on/off byte 一致・make test green(on)・MSRV 1.93 へ。残フェーズ =
      **J2 Tier A opcode 網羅（+ 残り 3 call 経路 + jit-stress CI）** → J3 呼び出し規約/IC →
      ...
      J2 の進め方: `MUTSU_JIT=on MUTSU_VM_STATS=1` の bailout opcode histogram を頻度順に潰す。
...
- **Lever 3: threaded dispatch — 凍結**（2026-07-06 ユーザー承認・[ADR-0004](docs/adr/0004-jit-strategy.md) §2.5 J0）:
      ...
      JIT が頓挫した場合のみ復活。
- [ ] **Lever 4: JIT（Cranelift）= ADR-0001 層4 — J1 骨組み完了（2026-07-12 #4471）、
      現行 = J2 Tier A 網羅** / Lever 5: 型制約チェックの tight-loop 省略。

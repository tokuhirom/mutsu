...
- [~] **Slice 5 collapse proper（次の本丸・着手中）**: per-call **scoped/overlay env**（呼び出しフレーム
    ...
    - **全パス変換チェックリスト**（各スライスは `make test` + 関連 roast をローカル検証、全 roast は CI）:
      ...
      - [~] `call_compiled_function_named`（heavy path）— **revert済・要再挑戦**。overlay の親フォールスルーが
            `:=` バインディングと衝突: `for @t -> \d,\seed,\endpoint,\result { ts(...) }` から raw param を持つ
            sub を呼び、body の `my $result; $result := list.List`（loop var `\result` と bare 名衝突 + 4-way
            destructure）で「Cannot modify an immutable value」。`resolve_binding`/sigil-strip 名前解決が
            overlay 親の immutable sigilless 変数へ解決される。再現 `tmp/min3.raku`、`roast/S03-sequence/
            exhaustive.t` で検出。正しく直すまで defer（correctness が理由・ROI ではない）。
      - [x] `call_compiled_method`（method 非 fast path）
      - [ ] `call_compiled_closure` / `vm_closure_dispatch`（closure capture/writeback）
      - [ ] `force_lazy_list_vm` / `_n`（gather/lazy-list）
      - [ ] **最終**: `env_dirty`/`locals_dirty`/`locals_dirty_slots` と `ensure_env_synced`/
            `sync_locals_from_env` の双方向 sync を撤廃（全パスが overlay 隔離されたら不要になる）。

...
- [x] **upvalue 化 Slice 2: 共有 `ContainerRef` セルで intra-iteration mutation を正しく**（PR #2669）。
      box-on-capture: クロージャ生成時、同一スコープ（`code.locals` に slot を持つ）の被捕捉スカラを
      slot+env 両方で `ContainerRef` に置換してから env をスナップショット → Arc 共有で捕捉後変異が見える
      （`for 1 { my $x=1; my $c={$x}; $x=2; $c() }` → `2`）。per-iteration freshness は vardecl 時に slot の
      stale ContainerRef もクリアして維持。付随で Pre{Inc,Dec} の ContainerRef deref 欠落と for-loop rw
      writeback の `$pair.value` clobber（pointy-rw.t）も修正。pin `t/closure-container-capture.t`。
- [x] **本丸の残: 兄弟クロージャの捕捉変数共有**（PR #2669）。`my ($g,$s)=make(); $s(42); $g()` → `42`。
      box-on-capture で同一レキシカルを共有 `ContainerRef` セルにしたことで根治（loop 外も対応）。
- [ ] **残（別根）: body-local `my` の env leak**（`for 1..3 { my $x=$_ }; say $x` が raku 未宣言エラー、
      ...
      コンテナ化とは別根。Slice 3 として別 PR で。outer-frame 由来（同一スコープ slot を持たない）の
      被捕捉自由変数の box も未対応 — cross-frame の変異/共有ケースが出たら box_captured_lexicals を拡張。

...
- [x] **`EVAL` / symbolic deref / `CALLER::` — carrier として分類しレバー A 終着（詳細は
      [docs/vm-decoupling.md](docs/vm-decoupling.md)「Lever A finish」）**。**tree-walk ではない**:
      `EVAL`/`EVALFILE` は既に `eval_block_value` で compile→バイトコード→サブ VM 実行
      （`run_compiled_block` が `VM::new(interp)`）し、eval 対象コードは tree-walk されない。
      pseudo-package 読み（`CALLER::`/`OUTER::`/`SETTING::`/`DYNAMIC::`）も reflective な env lookup で
      AST 実行ではない。両者は interpreter を**キャリア**として使うだけ（compile/lookup に必要な
      env/classes/roles レジストリを所有しているため）なので、これらをフォールバック計上するのは
      計測ノイズだった。`VM::is_interpreter_carrier_function` で判定し、2つの interpreter 入口
      （`call_function_compiled_first` と `dispatch_func_call_inner` の最終 else）で
      `record_function_carrier` に振り分け。`MUTSU_VM_STATS=1` は carrier を別バケットで報告
      （`interpreter_carrier=N` + `function-carrier by name`）。t/ 集計で関数 tree-walk-fallback
      18.8%→18.6%（EVAL 54 件が carrier へ移動）。**残るフォールバックの支配項は
      `__mutsu_atomic_post_inc_var`（atomic `$x⚛++`）= concurrency/共有状態 = レバー B 領域**であり
      レバー A 対象外。`EVAL` の残結合（共有 env/レジストリが Interpreter 構造体所有）は明示的にレバー B
      の課題。pin `cargo test carrier_functions_are_not_tree_walk_fallbacks`。
...
- [~] **Slice 5 collapse proper（次の本丸・着手中）**: per-call **scoped/overlay env**（呼び出しフレーム
    ...
    - **全パス変換チェックリスト**（各スライスは `make test` + 関連 roast をローカル検証、全 roast は CI）:
      ...
      - [ ] **Slice 6.3（最後のフラグ = レバー B 完遂）**: `env_dirty` → by-name env 書き込み（tree-walking
            interpreter or by-name VM var op）が local slot に alias するキーを変更した後の slot 再 sync 用。
            consumer は `ensure_locals_synced`(env_dirty gate)→`sync_locals_from_env`(O(locals) env→locals pull)。
            **次セッションはここから着手。実行可能なブロッカー地図（164 setter / 少数 consumer の分類・段階撤去計画）を
            docs/vm-dual-store.md Slice 6.3 に整備済み（2026-06-07）**。要点: setter は (1)dispatch 後の保守的マーク
            ~92（method_mut47/method32/func13、multi-tier merge が captured-outer write を既に精密 signaling 済み＝
            blanket set を条件化するのが最初の的）、(2)by-name var/control op ~36（reverse write-through で slot 同期）、
            (3)I/O+carrier ~21（EVAL/format/atomic の interpreter bridge 撤去に gated、Say/Put/Print/Note の
            full round-trip は独立に無駄）。段階撤去: ①保守的マーク条件化 → ②by-name op の reverse write-through →
            ③carrier bridge 撤去で消滅 → env_dirty/ensure_locals_synced/sync_locals_from_env/saved_env_dirty 削除。
            metric: `MUTSU_VM_STATS=1` の `locals_pulls`→0（mb_method/mb_nested、出力不変）。
            **計測で確認した残コスト2点（2026-06-07、詳細は docs/vm-dual-store.md「Remaining method-call dual-store costs」）**:
            (1) メソッド呼びごとの `locals_pulls`（= env_dirty。opcode が無条件 set）。**caller env overlay の Arc 比較で
            条件化を試みたが失敗・revert**（Arc 保持が refcount を上げ make_mut deep copy を誘発、かつ caller env Arc は
            毎呼び変わるので env_changed が常に true）。= 本質的に Slice 6.3 / interpreter 切り離し待ち。
            (2) **ネストしたメソッド呼びの O(env) deep copy — 解消済み（multi-tier overlay #2684）**。
            `Env::parent` を単一 flattened overlay (`Option<Arc<HashMap>>`) → **親 env まるごとのチェーン**
            (`Option<Arc<Env>>`) に変更し、`scoped_child` が caller env を丸ごとチェーン。scoped 下のメソッドが
            別メソッドを呼んでも flatten 不要。さらに第2の deep copy（`merge_method_env` の `saved.clone()+insert`
            が親 Arc の overlay 共有で毎 return `make_mut`）も、`merge_method_env` を owned 受け＋callee 書き込み Vec
            収集 → `current` drop → in-place 変異、で撲滅。`mb_nested` で `env_deep_copies` 10001→1（出力不変）。
            長命キャプチャは capture サイトで `clone_env()` flatten。pin `t/multitier-overlay-env.t`。
            docs/vm-dual-store.md 項目2。
...
- [x] **upvalue 化 Slice 2b: コンパイラ解析で boxing を「捕捉され、かつ宣言以外で変異される loop-body local」に精密化**。
      `CompiledCode.captured_mutated_locals`（nested closure の `free_var_syms` ∩ own local かつ、再代入
      /inc-dec/nested closure からの write がある local）を算出（`free_var_writes` も併設して fold）。宣言と
      再代入は両方 `SetLocal` だが前置 `MarkVarDeclContext` で区別。box_captured_lexicals を `loop_local_vars`
      ∩ `captured_mutated_locals` に限定 → read-only ループ local は owned_captures に委ね無駄な box を回避。
      さらに closure dispatch で `data.env` の ContainerRef を merge 時に**上書き**（共有セル優先）し
      cap_overrides をスキップ → ループ local の**繰り返し変異**が正しく見える
      （`for 1 { my $x=1; my $c={$x}; $x=2; $c(); $x=5; $c() }` → `2,5`）。pin に当該ケース追加。
- [ ] **残（延期・大）: 非ループの一般コンテナ捕捉**（`my ($g,$s)=make(); $s(42); $g()` → raku `42`、
      `my $x=1; my $c={$x}; $x=2; $c()` top-level → `2`）。コンパイラ解析（captured_mutated_locals）は非ループも
      検出できるが、**非ループ被捕捉スカラを box すると ContainerRef 未対応の多数経路を踏む**。今セッションで
      具体的に確認したギャップ: 型付き `my A $a` の `.=`（型消失 + write 不達）、isa-ok 経由（`.=` 下流）、
      subset 型オブジェクト代入（subset-6e #54）、submethod state（submethods #12,16,18,19）、Mix 不変性。
      これらは「値検査サイトに ContainerRef が漏れる」共通根で、全 lvalue/型/dispatch 経路の deref 監査
      （dual-store/コンテナ・アーキテクチャの本丸）が要る。専用の複数 PR でやる。`t/closure-container-capture.t`
      の非ループ兄弟 2 ケースは `todo`。
- [x] **Slice 3: ループ本体ローカル `my` の clobber（クロージャ非依存の正しさバグ）**。`my $x=99; for/while/loop/
      repeat { my $x=... }; say $x` が外側 `$x` を**クロバー**して最終値（raku は `99`）になる正しさバグを根治。
      dual-store の env が name-keyed で、ループ本体が全 local を `needs_env_sync` 化するため、内側 `my $x`
      の write-through が env["x"] と共有スロットの両方で外側を上書きしていた。**fresh-slot（コンパイラ）案は
      却下**: 同名で複数スロットを作ると name-based の `captured_mutated_locals`/box-on-capture（lever C）が
      ブロックを跨いで誤適用し closure 捕捉が壊れる（name-keyed env アーキテクチャと衝突）。**採用＝ランタイム
      復元**: ループ本体で**既存の外側束縛を shadow した** `my` 名のみ、宣言時に shadow した外側値を
      `loop_local_saved_env` に記録し、ループ脱出で env＋共有スロットへ復元（`GetLocal` fast path が非Nilスロットを
      env無視で返すため両方必要）。ループ**条件**の `my`（`... until COND` 文末修飾含む）は enclosing-scoped で
      ループ後に読まれるため `loop_cond_active` で記録対象から除外（`t/iterator-regressions.t` 回帰を回避）。
      pin `t/block-local-my-scope.t`。**残**: 外側束縛が無い純粋 leak（`for {my $x=...}; say $x`）の
      post-loop 読みは raku 未宣言エラーだが mutsu は undefined 値を返す（strict-vars 未実装＝`$neverdeclared` も
      Nil で全体的に未強制。正しさ実害なし）。
- [ ] **Slice 3b（残）: `if`/`else` ボディローカル `my` の clobber**（`my $x=99; if c { my $x=5 }; say $x` が
      raku `99`、mutsu は `5`）。当初 `if`/`else` を BlockScope（完全 env+locals save/restore）でラップしたが、
      これは**ブランチ内から外側変数への `:=` 束縛を巻き戻す**回帰を生んだ（`roast/S03-sequence/exhaustive.t` の
      `$result := @result[0].List` を捨てて 27 subtest 失敗。`t/block-local-my-scope.t` の if/else ケースは todo）。
      正しい解＝loop と同じ shadow-only 復元を `if` ブランチにも効かせる軽量な block-local-scope 機構
      （`PushBlockLocalScope`/`PopBlockLocalScope` opcode で `loop_local_saved_env` を流用、BlockScope の全 env
      復元は使わない）。別 PR で。
...
**レバー A は終着**（全項目 `[x]`）。残る VM↔Interpreter 結合はレバー B（状態オーナーシップ）と
レバー C（upvalue / 第一級コンテナ）に収斂する。**次の着手候補（優先順）:** B の Slice 6.3
（最後のフラグ `env_dirty` 撤去、VM decoupling 完了に gated）→ C の非ループ一般コンテナ捕捉
／body-local `my` の env leak（Slice 3）→ 🟣第2優先「第一級コンテナ」Phase 0。

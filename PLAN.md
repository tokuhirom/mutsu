...
- [ ] **Slice 6.3 残り（= `env_dirty` フラグ削除 = レバー B 完遂）**。フラグはまだ存在し、稀な
      ...
      - (B) **残り step2（次の着手）**: 他の by-name var/control op（vm_var_get_ops / vm_register_ops の `&sub`・型名 / vm_control_ops / vm_data_ops / vm_var_assign_ops / vm_misc_ops）。同じ reverse-write-through パターン。`grep -n "env_dirty = true" src/vm/*.rs` で setter 列挙。

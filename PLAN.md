...
- **§2 Phase B** は層3a（GC）・層3b（NaN-boxing）完了、層4 JIT は J1〜J5 完了で
  **既定 on**（2026-07-13・ADR-0004）— 残スライスは J4d（Tier B 変数 op インライン）。
...
| **C. JIT** | 独自メリット（**J1〜J5 完了・既定 on**（2026-07-13）— 残 = J4d） | §5 Lever 4 |
...
- **Lever 3: threaded dispatch — 凍結**（2026-07-06 ユーザー承認・[ADR-0004](docs/adr/0004-jit-strategy.md) §2.5 J0）:
      ...
      JIT が頓挫した場合のみ復活。
- [ ] **Lever 4: JIT（Cranelift）= ADR-0001 層4 — J1〜J5 完了・既定 on（2026-07-13）、
      残 = J4d（Tier B 変数 op インライン）** / Lever 5: 型制約チェックの tight-loop 省略
      （J3 の `type_matches_value` fast accept で大半を回収済み）。
...
- [ ] **Lever 7: ベースライン（古典的）バイトコード最適化 = [ADR-0006](docs/adr/0006-baseline-interpreter-optimizations.md)**。
      ...
      - [ ] **§2.3-c 残 administrative op（`SetVarDynamic`・`CheckReadOnly`）** — 上記の
            事前計測ゲートを通ってから着手。
- [ ] **★perf の次の的は profile が示す「割当と call path」（opcode ヒストグラムではない）**:
      fib の profile 上位は ① `call_compiled_function_positional_light`（call path）
      ② `malloc`/`free`（**呼び出しごとの割当チャーン**）③ `Env::get_sym`（dual store の名前引き）。
      ② と ③ は locals↔env の dual store という同じ根から出ており、**§6 の
      「`BlockScope` の locals 全 clone/restore 撤去」＝レキシカルスコープ slot キャンペーン**
      （[docs/lexical-scope-slot-campaign.md](docs/lexical-scope-slot-campaign.md)）が直接叩く。
      これを perf の本丸として §6 から昇格させる。
...
| JIT | **default on ✅**（2026-07-13・ADR-0004 J5） | 達成（残 = J4d 変数 op インライン） |

...
| フェーズ | 内容 | 本書の該当 |
|---|---|---|
| **A. 追いつく** | 互換性＋速度で raku に並ぶ（**完了・ADR-0002**） | §F roast / §2 multi-dispatch / §H module / §G method-call hot-path・Lever 3 |
| **A'. 地ならし** | root 集約で GC 実装を楽にする | レキシカルスコープ（ANALYSIS §1.4）/ upvalue index 化（ANALYSIS §1.3） |
| **B. Value 表現リワーク＋GC** | Track B（要素 cell 化）＋ cycle collector を**統合**（ADR 層3a）（**いまここ**） | §G Lever 2 周辺 / §I Track B 依存項目 / §J |
| **C. JIT** | 独自メリット | §G Lever 4 |

- **GC は JIT の前**（JIT を GC 前提の上に載せる）。**Phase A は完了済み**（ADR-0002：roast 目標達成、
  残 perf レバーは GC 後に順序づけ済みのため Phase A 完了条件に含めない）。
...
- Level 1 の詳細設計メモは [docs/gc-level1-detailed-design.md](docs/gc-level1-detailed-design.md) を起点にする。
  **最初の実装スライスは root visitor 導入**（`Interpreter::visit_roots()` への root 集約、§11 手順1）。
...
- **Phase B 本丸（2026-07-03 着手・ADR-0002）**: §J の GC（cycle collector on Arc）。
  詳細は [docs/gc-level1-detailed-design.md](docs/gc-level1-detailed-design.md)。
...
- [ ] **残件**:
  ...
  対象は `state` / `EVAL` / `CATCH` / sigilless scalar（`\x`）/ `rw` / `raw` / 戻り型 coercion。
  現在は保守ゲートで除外しており、本筋は `compiled_fns` の拡充。
  - **完了済み（計測で確認）**: bare multi の OTF 化は compiled_fns 経由で実質完了
    （state 候補 / caching-proto body も #4047 で OTF 化）。`code_signature`（`&cb:(Int)`）param
    は #3883 で OTF 化済み（function / method / module / return-type 全経路で fallback 0；
    残る fallback は EVAL 内の署名不一致エラーパスのみ）。モジュール sub の capture param
    （`|c` / `|c($a,$b)` = sub-sig）も OTF 化済み（sigilless scalar と違い caller-alias
    writeback を持たない read-only 束縛のため許可）。
...
- [x] ~~**型付き例外**: `S32-exceptions/misc.t`~~ — **完了・whitelist 済み**（misc.t 182/182・misc2.t 265/265）。
      X::NotParametric / X::Undeclared / X::Redeclaration / X::Bind / X::TypeCheck 等の infra は実装済み。
      #4053（非コンテナへの element bind → X::Bind）・#4055（同一スコープ class 再宣言 → X::Redeclaration）で仕上げ。
      残る typed-exception の gap は全て非-trivial（strict-mode undeclared 変数検出が未実装／class 再宣言の
      cross-EVAL 検出は runtime + compilation-unit 追跡が必要／X::Redeclaration::Outer は compile-time scope 解析）で、
      単体では roast ファイルを whitelist 化しない。着手前に必ず `grep whitelist` + `prove` で現状を確認すること。
- [ ] **★lazy-seq（残 ④のみ・2026-06-29 更新）**: `1..*` 配列は reify-on-index 済、`(1...Inf)`=Seq is-lazy=True。
      ...
      `+a` Seq→Seq/`+@a` Seq→List）。④ の内訳: **reify-on-mutation の elem-assign/`:delete` は解決済**
      （`reify_lazy_array_slot` を全 `@`-array-context LazyList に拡張＝有限 `(1..10).lazy` は cache 全体、
      無限 spec は 100k cap で materialize してから書き戻す。以前は空 Array に autoviv して要素喪失）。
      残: 変異後の is-lazy 保持（partial-reify＋lazy tail・`docs/lazy-arrays.md` §4.1）／`push`/`pop` が lazy list で
      `X::…` を投げる raku 挙動／slurpy 真 lazy（値は正・is-lazy が False）／lazy `.gist`＝**解決済**。
      BLOCKERS.md §「Real lazy infinite sequences」。
- [x] ~~**Match キャプチャ番号付け / コンテナ kind**~~ — **完了・whitelist 済み**（`S05-capture/array-alias.t` 50/50）。
      (1) `$<x>=(...)` の positional スロット重複格納・(2) `m:g//` を `my @m` 代入後の `@m.gist` が `(…)` を返す問題は
      両方とも解決済み（raku と完全一致：named capture は positional に重複せず、`@m.gist` は `[…]` で elems 正）。
      S05-capture の残り非-whitelist は hash.t（`%<chars>=(...)` = rakudo 自身も未対応の 2004 年構文）のみ。
...
2. [ ] **ローカル完結コマンドの仕上げ**: `--help`/`info`/`locate` 等を完全動作。2 件の MAIN ディスパッチバグを修正済み:
   ...
   **CompUnit::Repository 調査（2026-07-02）で新たに判明した別軸ブロッカー2件**（実 zef バイナリの end-to-end
   実行を阻む・CompUnit::Repository 自体とは無関係）: (a) `Zef::Client` の `%`-sigil 属性に Associative を
   ダックタイピングする非 Hash オブジェクト（`has %.hash handles <AT-KEY EXISTS-KEY ...>`）を bind すると
   `coerce_attr_value_by_sigil`（`methods_signature.rs`）の catch-all を素通りして raw Instance のまま格納される。
   (b) 実 `Zef::CLI.rakumod` のコンパイルを止めるパーサエラー（未特定）。
3. **✅ 本丸: mutsu ネイティブ `CompUnit::Repository`（2026-07-02）**: install 済みモジュールが `use` できる橋。
   調査の結果 `FileSystem`/`Installation`/`install`/`need`/`resolve`/`use lib "inst#..."` は既に実装済みで、
   欠けていたのは橋の3か所のみ：① デフォルト `$*REPO` の属性欠落（`short-id` 死ぬ実バグ）、②
   `repository-for-name("site"/"home"/"vendor"/"perl")`（zef のインストーラが実際に呼ぶ経路）が well-known 名を
   扱えなかった、③ `.install()` 成功後も素の `use ModuleName`（`-I` なし）が見つからない — デフォルト site
   リポジトリを `resolve_module_path` の検索対象に自動登録していなかった。③ は `Interpreter::new()` 自体に
   `add_default_site_repo()` を組み込むことで解決（`main.rs` だけでは不十分 — `Test::Util` の `is_run` は
   in-process nested `Interpreter::new()` の高速パスを持ち `main.rs` を経由しない）。担保＝
   `t/compunit-repository-for-name.t`。詳細＝news/2026-07.md。
...
| 指標 | 現在 | 目標 |
|------|------|------|
| Whitelist | **1345**（全 .t 1463 中） | 1300+ ✅ 達成済み |
...
| zef CLI（`mzef` north-star） | **CLI 起動＋dispatch ✅／CompUnit::Repository install→use 橋 ✅（2026-07-02）** | 実 zef バイナリでの実 install（残 2 バグ） |

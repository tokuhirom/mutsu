...
- [~] **Slice 5 collapse proper（次の本丸・着手中）**: per-call **scoped/overlay env**（呼び出しフレーム
    ...
    - **全パス変換チェックリスト**（各スライスは `make test` + 関連 roast をローカル検証、全 roast は CI）:
      ...
      - [ ] `call_compiled_function_positional_light`（name-based param-env juggling を overlay drop に置換）
      - [ ] `call_compiled_function_light`（named-param light）
      - [ ] `call_compiled_function_named`（heavy path、caller_env/sub_val/rw bindings 込み）
      - [ ] `call_compiled_method`（method 非 fast path）

...
- [ ] **map の残**: ~~Pair 要素（`$_` 明示バインド）~~ **済**: native map が Pair 要素でも `$_`/単一
      plain positional を明示バインド（`call_compiled_closure_with_topic`）。implicit-`$_` ブロックと
      pointy `-> $p` を native 化。placeholder (`$^a`) / multi-arity over pairs は引き続きフォールバック
      (bind が "Missing required placeholder" を投げるため)。残: `$_` mutation 書き戻し（rw エイリアス）
      — native 経路にターゲット変数名が届かず `.grep` rw ビューと同根、後回し。

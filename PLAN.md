...
- **レバー C（クロージャ upvalue 化）= 完了**: スライス 1/2/2b/3/3b に加え、残っていた非ループ一般コンテナ
  捕捉も Phase 1 第1スライスで解消（兄弟クロージャがサブ本体ローカルの `ContainerRef` セルを共有。型制約
  スカラーのみ box 対象外＝広範 write 監査へ後送り）。詳細は [docs/container-identity.md](docs/container-identity.md)。
...
- [~] **Phase 0 — decont チョークポイント整備（挙動不変リファクタ）**: 実装ログは
  ...
  - [~] **Phase 0.5 = スタック不変条件＋lvalue opcode（段階的に分割）**: 値スタックを「常に decont 済み」
    ...
    - [ ] **第2段（実挙動変化を含む本体）**: `GetArrayVar`/`Index` の auto-decont（= stack 不変条件）と新
          ...
          **同一 PR** で入れる。
- [~] **Phase 1 — スカラーの第一級コンテナ化**: `$` 変数が共有セル（`ContainerRef`）を持つ。`=` はセルへ格納、
      `:=` は束縛差し替え、itemization はセル wrap。
  - [x] **第1スライス（非ループ兄弟クロージャのコンテナ共有 = レバー C 完遂）**: `box_captured_lexicals` の
        ループ限定ゲートを撤去し、非ループの捕捉＋変異 `$` スカラーも共有セル化。値漏れ 1 箇所
        （メソッド fast-path の Var 読み）を `into_deref` で修正、型制約スカラーは box 対象外にして制約再チェックを
        保持。`my ($g,$s)=make(); $s(42); $g()` が共有されるように。詳細は
        [docs/container-identity.md](docs/container-identity.md)。
  - [ ] **残**: `.VAR.^name` が束縛コンテナ型を反映 / `is rw` スカラーの caller エイリアス（S03-binding/scalars.t
        24,27） / 束縛/平坦化（reduce.t 62）/ S02 変数捕捉。

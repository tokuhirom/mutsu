...
1. **高速起動の互換インタープリタ** — 起動 raku 比 0.04x・roast whitelist 1373。CLI ツールと
...
- **§2 Phase B** は層3a（GC）完了・層3b（NaN-boxing）が次の大型スライス（ADR-0005 待ち）。
...
| **B. Value 表現リワーク＋GC** | 層3a（Track B＋cycle collector 統合）**完了**・層3b NaN-boxing（**いまここ**・ADR-0005 待ち） | §2 |
...
## 2. ★ Phase B: GC → NaN-boxing → JIT（層3a 完了・層3b 進行中）

**GC（cycle collector on Arc・層3a）は完了・既定 on**（2026-07-05 ADR-0003）。§11 全 step・
cross-thread cooperative STW・Track B 要素セル化（T1–T6）・candidate buffer `Weak` 化・監査性
sweep まで含めて完了 — 経緯と詳細は [news/2026-07.md](news/2026-07.md)（層3a 完了サマリのアーカイブ
エントリあり）。残:
...
- [ ] **Lever 2: NaN-boxing = ADR-0001 層3b（JIT の地ならし・GC 後）**: `Value` 48→8 bytes。
      ...
      スライス計画とゲート = [docs/gc-post-3a-roadmap.md](docs/gc-post-3a-roadmap.md) §3、
      API 壁 = [docs/nanbox-3b0-api-wall.md](docs/nanbox-3b0-api-wall.md)（ratchet は残置）。

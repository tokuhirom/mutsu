...
- **§2 Phase B** は層3a（GC）・層3b（NaN-boxing）完了、層4 JIT は J1〜J4 コア
  （#4471/#4474/#4476/#4478/#4479/#4480）完了 — 次スライスは J4d（Tier B 変数 op
  インライン）ないし J5（既定 on）判定（ADR-0004）。
...
| **C. JIT** | 独自メリット（**いまここ** — J1 #4471・J2 #4474・J3 #4476 完了・次は J4） | §5 Lever 4 |
...
- [ ] **層4 JIT（Cranelift・= §5 Lever 4）**: **J1〜J4 コア完了**（J1 #4471・J2 #4474・
      J3 #4476・**J4 = Tier B インライン算術 #4478 + hot-loop entry #4479 + 変数 op
      ホットパス #4480**、2026-07-12〜13・詳細 = news/2026-07.md）。J4 で
      Add/Sub/Mul/比較の NaN-box タグ分岐 CLIF 直接展開・`run_range` フックによる
      compound loop body の range JIT（int-arith 級 top-level ループが初めて native 化）・
      CheckReadOnly/GetGlobal/GetLocal/SetLocal の per-iteration format!/intern 除去
      （インタプリタ側も -34%）。ゲート: fib ループ `gc_candidate_pushes==0`・inline 部
      refcount ゼロ ✅ / 「int ループ 5-10x」は文言上未達（インタプリタ自体が高速化して
      相対差が縮小 — 残りは変数 op の helper call）。残フェーズ:
      **J4d（Tier B 変数 op インライン: GetLocal/SetLocal fast path・JIT→JIT per-call
      bind インライン・args Vec alloc 除去 — 5-10x ゲートの本体）** → **J5 既定 on**
      （gc-stress × jit-stress green ✅ 済・起動予算とベンチ非回帰を bench-data の
      `+jit` 系列で確認して切替）= [ADR-0004](docs/adr/0004-jit-strategy.md)。
      ベンチ数字の正本は bench CI（`bench-data` ブランチ・`+jit` 系列は #4480 から）。
...
- **Lever 3: threaded dispatch — 凍結**（2026-07-06 ユーザー承認・[ADR-0004](docs/adr/0004-jit-strategy.md) §2.5 J0）:
      ...
      JIT が頓挫した場合のみ復活。
- [ ] **Lever 4: JIT（Cranelift）= ADR-0001 層4 — J1〜J4 コア完了
      （#4471/#4474/#4476/#4478/#4479/#4480）、次 = J4d（Tier B 変数 op インライン）
      ないし J5 既定 on 判定** / Lever 5: 型制約チェックの tight-loop 省略（J3 の
      `type_matches_value` fast accept で大半を回収済み）。
      残フェーズとゲートは **[ADR-0004](docs/adr/0004-jit-strategy.md)**（詳細 = §2 /

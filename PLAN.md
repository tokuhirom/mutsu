...
- [x] **`.new`（デフォルトコンストラクタ）の単純構築 fast path を VM へ** (Step 5)。
      「カスタム new/BUILD/TWEAK なし・単純 `$` 属性・継承チェーンも全て単純」なクラスの
      デフォルト構築を `try_native_default_construct` に抽出し VM から直接呼ぶ。
      method-call.raku 20%→0%、bench-class 13.8%→0%（両ベンチ method fallback ゼロ）。
      残る大物は BUILD/TWEAK/型強制を含むクラスで、クラスレジストリ/BUILD/TWEAK を
      VM 所有データへ移す必要（§1.1）。
- [~] **クロージャ/正規表現を取るメソッド** (`sort` の比較子, `.subst`, `map`/`grep` のブロック)。
      **重要な注意（2026-06 調査）**: これらの「フォールバック」は**ブロック本体ではなく orchestration の入口のみ**。
      ブロック本体は既に `eval_map_over_items` で compile→`run_compiled_block`(`VM::new`) され VM 上で
      コンパイル実行されている（ツリーウォークではない）。フォールバックしているのは map/grep/sort の
      **反復ループ・arity チャンク・Slip 平坦化・LAST phaser・lazy-return 判定・capture 永続化**という
      薄い Rust orchestration 部分。したがって native 化は **ツリーウォーク削減ではなく計測上の
      フォールバック率改善**であり、`.new`（interpreter の実行経路 dispatch_new を除去した）とは性質が違う。
  - [x] **`.map`** を VM native 化（`vm_native_map.rs` の `try_native_array_map`）。real array・単一 `Sub`・
        arity 1・full-binding 不要・body スキャナで escape（return/last/take/phaser）と `$_` mutation
        （`$_=`/`s///`/`tr///`/`$_++`）と Pair 要素を弾く、を満たす場合に `vm_call_on_value` で VM 内反復。
        それ以外は全フォールバック。probe の map fallback を 50→0 に。
  - [ ] **`.grep`**: 結果が**元配列への rw ビュー**（`@a.grep(...)>>++` が元を更新）で、clone ベースの
        native 経路では再現できないため後回し。rw aggregate binding を VM で扱う設計が要る。
  - [ ] **`sort` 比較子 / `.subst` ブロック**: 同様に段階導入（比較子は 2-arg・安定ソート、subst は regex+置換）。
  - [ ] map の残: 多 arity（`-> $a,$b`）チャンク、Pair 要素（`$_` 明示バインド）、`$_` mutation 書き戻し。
- [ ] **Test 関数** (`is`/`ok`/`plan`/`is-deeply`/`subtest`…) = roast の残フォールバックの大半。
      TAP `TestState` を VM から到達可能にする必要（大物・後回し）。これは（上の map/grep と違い）
      ブロック本体ではなく **状態（TAP カウンタ）の所有が interpreter 側**である点が本質で、レバー B 寄り。

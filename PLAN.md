...
現状（2026-06-28 再計測・PERFORMANCE.md）: 起動 0.04x。method-call ≈ 1.8–2.1x。
...
- [ ] **method-call ホットパス整理（新・計測駆動）= method-call <2x 継続**:
      - [x] per-call body-fingerprint 撤去（#3853）: `push_method_dispatch_frame` が候補同定のため毎回メソッド body
            AST 全体を `format!("{:?}")` 化していた→単一候補早期 return ＋ alloc-free fingerprint。method-call 0.44→0.32s（~27%）。
      - [ ] **次: symbol ラウンドトリップ除去**: dispatch が `Value::Package(Symbol)` を `resolve()`（String 確保）→
            `intern()`（FxHashMap ルックアップ）で Symbol に戻している。元 Symbol を再利用し `&str` ベース dispatch API を
            Symbol 化（`Symbol::intern`+`memcmp` が method-call の ~14%）。
      - [ ] `Value` clone/drop（bench-class の ~50%）は Lever 2（NaN-boxing・GC 後）待ち。

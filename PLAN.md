...
- [~] **Test 関数の TAP 状態所有を Interpreter god-struct から分離（レバー B 寄り・大物）**: #2625 は
      **ディスパッチ層のみ**。本体 (`is`/`ok`/`is-deeply`/`subtest`…) は TAP カウンタ・TAP 出力・method
      dispatch (`.gist`)・`EVAL`・subprocess に本質的に結合。
    ...
    - **Step A 済（このPR）**: 4つの生フィールド（`test_state`/`subtest_depth`/`subtest_callable_is_sub`/
    ...
    - **Step B 残**: `tap: TapState` フィールドの**所有形態だけ**を差し替える（`Rc<RefCell<TapState>>` 化
      または Interpreter 外のセルへ）。アクセスが `TapState` の API に一点集約されたので、B は型 + 中央
      アクセサ数本の変更で済み、~50箇所の呼び出し側には再度触らない。`&mut TestState` を返す
      `ensure_state`/`state_mut` だけが RefCell 化時の borrow 再スコープ箇所（既知・局所）。
...
- [~] **Slice 5 collapse proper（次の本丸・着手中）**: per-call **scoped/overlay env**（呼び出しフレーム
    ...
    - **全パス変換チェックリスト**（各スライスは `make test` + 関連 roast をローカル検証、全 roast は CI）:
      ...
      - [ ] **Slice 6.2+（最終・残作業, 2分割）**: dirty フラグの完全撤廃。
            - **`locals_dirty` → write-through**: `needs_env_sync` slot の書き込みを遅延 flush でなく eager に env へ
              ミラーし、`locals_dirty`/`locals_dirty_slots`/`ensure_env_synced` を削除。12 ファイルに散在する
              ~82 の直接 `self.locals[idx]=…`（遅延 flush 依存）+ ~24 `mark_local_dirty` の監査が前提。
            - **`env_dirty` → bridge 撤去依存**: tree-walking interpreter が env を名前で変更した後の slot 再 sync 用。

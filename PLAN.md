...
- [ ] **Slice 6.3 残り（= `env_dirty` フラグ削除 = レバー B 完遂）**。フラグはまだ存在し、稀な
      ...
      - (A') **regex/smartmatch の pull — 延期（unsafe）**。`exec_smart_match_expr_op` の blanket `env_dirty=true` を
        外す案を試みたが回帰（`S02-magicals/sub.t` の `regex foo { a { $var = &?ROUTINE } }`、`S05-modifier/pos.t` の
        `:pos` 継続マッチ）。**理由**: 正規表現 RHS は埋め込みコードブロックや pos 状態を通じて**任意の caller local を
        interpreter regex エンジン経由で名前書き込み**し、これは env_dirty を立てない。安全化には regex op が
        「副作用あり（埋め込みブロック/pos/adverb）」を精密にシグナルする必要がある（regex 値に has_code_blocks 等の
        フラグ）。単純 regex（`/\d+/`）と区別できれば bench-string の win を取り戻せる。専用タスク。

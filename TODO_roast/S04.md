...
- [ ] roast/S04-declarations/my-6e.t
  ...
  - **Progress (2026-06-30, slice 2): 106/109 (was 99). 7 more fixed:**
    - 3,4: use-before-declaration in the *same statement* (`$ret = $foo ~ my
      $foo`) now throws `X::Undeclared`. The EVAL undeclared-var pre-pass
      (`find_undeclared_var_in_stmt`) gained a `Stmt::Assign` arm that walks the
      RHS left-to-right (`find_undeclared_var_ordered`), so an embedded `my $x`
      only shadows references *after* it (`(my $x) ~ $x` stays legal).
    - 52: EVAL of a later-declared caller lexical (`eval_elsewhere('!$y.defined')`).
      Root cause was an EVAL `my`-leak: a `my` declared inside EVAL'd code leaked
      into the caller pad, so earlier loop/comma `EVAL 'my $y'` left `$y` defined.
      Fixed in `parse_and_eval_with_operators`: snapshot the plain-user-lexical
      env keys before running and drop any new ones after (`is_plain_user_lexical`).
      Also extended `collect_declared_vars` to see `my` in loop-init / if / while
      conditions so the undeclared-var pre-pass no longer false-flags them.
    - 90: EVAL of code calling an undeclared routine now dies at CHECK time. New
      `check_eval_undeclared_routines` pre-pass flags a bareword call whose name
      is not a declared sub / builtin / proto / `&name` in scope.
    - 94: anonymous `my &` now defaults to the `Callable` type object (compiler
      `stmt.rs` + `expr_block.rs`: `Nil`-init `&`-sigil decl → `Callable`).
    - 103: `my sub {42}()` anonymous-sub immediate-call now parses
      (`my_decl_dispatch.rs`: `my sub` followed by `{`/`(` → parse as an
      expression so postfix `()` applies).
    - 107: nested sigilless destructuring assignment
      (`my (\d, (\e, (\f, (\g, \h)))) = ...`) now parses + runs without throwing

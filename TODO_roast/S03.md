...
- [ ] roast/S03-metaops/hyper.t
  - 5 sub-subtest fails (was ~20, was 154). Fixed (latest pass): the parser
    ...
    the first occurrence (`hyper_concat.rs`'s closing-`>>`-marker search now
    tries all candidate positions instead of bailing on the first, possibly
    overlapping, one); a Pair leaf now broadcasts a hyper op into its `.value`
    keeping `.key` (rakudo#4700); `.eager`/`.tree` nodal-vs-recursive
    dispatch corrected (`.eager` now duckmaps to true leaves and a scalar's
    `.eager` wraps into a one-element List; `.tree` is nodal since it already
    recurses internally); `».[a;b]` multi-dim hyper indexing now passes each
    dimension as its own `AT-POS` arg instead of one `ArrayLiteral` (matches
    the non-hyper `@a[a;b]` `MultiDimIndex` desugar); `.roll(*)` is now a
    genuine infinite lazy Seq (`SequenceSpec::RollPool`) instead of an eagerly
    generated 1024/131072-item prefix, so it renders `(...)` and pulls
    indefinitely; a **general, non-hyper-specific** bug — `set_shared_var`
    (backing every `@`/`%` env write via `set_env_with_main_alias`, not just
    threaded code) unconditionally normalized `List` to `Array`, silently
    corrupting a `:=`-bound List's kind on the *first* subsequent env sync
    even in single-threaded scripts; the normalization is now gated to only
    apply when writing into the actual cross-thread shared-var store; a
    hyper method call with a qualified name (`».Any::elems`) now parses the
    full qualified name (was split into a bare `».Any` hyper call plus a
    stray `elems` bareword statement) and nodality lookup strips the
    `Owner::` prefix before matching against the list-native-method set;
    `X::Seq::Consumed` now carries a `.kind` attribute (`Seq`/`HyperSeq`/
    `RaceSeq`); `HyperSeq`/`RaceSeq` are recognized builtin type names so a
    bareword `HyperSeq` resolves to the type object instead of an implicit
    string (fixes `kind => HyperSeq` matcher comparisons); a typed source
    array's hyper-op result degrades from `Array[T]` to a plain `List` when a
    result value doesn't satisfy `T` (rakudo#5778), instead of forcing
    `ArrayKind::Array` unconditionally.
  - Older fixes: nodal hyper methods return a `List`;

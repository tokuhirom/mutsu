...
- [ ] roast/S02-types/range.t
  - 251/259 pass (was ~203). **2026-06-29: min/max with `:k`/`:v`/`:kv`/`:p`
    adverbs implemented (48 tests fixed).** `Range.min`/`.max` (and the sub forms
    `min($r,:k)` / `max($r,:kv)`) now report the index of the extremum element:
    a Range's min is its first element (index 0), its max its last (index
    `elems-1`, or `Inf` for `n..Inf` where the value is also `Inf`). Fixes:
    (1) `extract_extrema_adverbs` strips `:!k`/`:!kv`/`:!p` (Bool-false) from the
    candidate list so they no longer pollute the comparison; (2)
    `apply_extrema_adverb` returns a SCALAR (first matching index) instead of a
    Seq of all matches, with `:kv`→`(k,v)` List and `:p`→`Pair(k,v)`; (3)
    `try_native_extrema` (VM) defers Ranges to the slow path; (4)
    `dispatch_min_max_method` handles Range+adverb analytically (value via the
    Inf-aware 0-arg path, index via `.elems`); (5) `builtin_min`/`builtin_max`
    delegate a single-Range sub-form call to the same path. t/range-minmax-adverbs.t.
  - Remaining 8 fails (8, 11, 191-192, 206-207, 210-211): Range-stays-itemized on
    `my @r = $range` (`@r.raku` == `[1..5,]`) and Inf/NaN endpoint edge cases
    (`.int-bounds`, `Inf..Inf`/`Inf..NaN` produce Nils, `+(-∞..-∞)`) — separate
    from min/max.

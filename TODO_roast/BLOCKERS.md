...
- **優先度**: 最優先
- **難度**: Hard
- **現状**: `TODO_roast/S32.md` ベースで 165/180 pass まで前進済み
- **理由**:
...
- **Next slice**:
  slice 1 (compile-sorrow accumulator, tests 20+26) と slice 2 (source-format
  preserving warning) 完了。slice 2 は透過 AST ノード `Expr::LiteralSrc(Value,
  source)` で数値リテラルの元書式を保持し、sink warning（tests 139-142）と
  `X::Syntax::Confused` の `pre`/`post`（test 17、`ORIGINAL_SOURCE` ポインタ演算
  で復元）を解決。残るは:
  (a) colonpair `:foo(42)` の source 保持（test 121）— AST が `Binary FatArrow` で
      `foo => 42` と区別不能、~29 の pair 検出箇所に影響するため別スライス。
  (b) BEGIN textual-time 実行（slice 3, `X::Comp::BeginTime`）。
  (c) 純 parser / control-flow one-off（test 3/16/21/22/27/84/155/157/161）。
- **Canary tests**:

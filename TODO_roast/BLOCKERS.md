...
| `S32-hash/perl.t` | 47/55・notok 8 | **55/55 満点** | per-holder hash itemization 実装で Scalar/decont 区別（12 subtests）解決。残 8 は匿名 typed hash `$(my Any %)` の EVAL round-trip 型喪失＝パラメータ化ロール type-capture binding 待ち（匿名 typed 容器タグ付けが binding を壊す既存制限） |
...
**このバッチで完了した達成可能ターゲット**（raku 満点だった実バグ、2026-07-11）:
- `S32-hash/perl.t` 43→47/55（#4421 per-holder hash itemization）。残 8 はパラメータ化ロール
  type-capture binding 待ち（上表参照）。
- `6.c/S14-roles/attributes.t` 2→3/3・whitelist 済み（#4425 クラス属性アクセサをロール
  合成メソッドより優先）。
- `6.c/S03-operators/set_precedes.t` 0→40/40・whitelist 済み（#4424 baggy set 演算子
  `(<+) ≼ (>+) ≽`）。
- mutsu が既に PASS していた 4 本を whitelist（#4423: `6.c/MISC/misc-6.c.t`,
  `integration/advent2010-day04.t`, `integration/advent2013-day19.t`,
  `integration/lazy-bentley-generator.t`）。

**残る達成可能 ★は `S12-attributes/class.t` のみ**だが、これは単一 fix ではなく複数機能
（クラス内 `BEGIN EVAL` によるコンパイル時属性宣言 test20、`EVAL` attr→NotFound test21、
`is_run` エラーメッセージ test22-23、`where` 節での属性アクセス `X::Syntax::NoSelf`
test24-28）が必要で、深い。他の非★残件も 6.e 待ち・6.d 制限・通過不能・深い多機能
（grammar エンジン: `6.c/S05-grammar/methods.t` の die 伝播 / 埋め込みブロックの外側
レキシカルキャプチャ / dup-token エラー、`6.c/S06-other/main-refactored.t` の新 MAIN
インターフェース全体）で、いずれも単発 quick win ではない。全体優先は引き続き
[PLAN.md](../PLAN.md) の構造工事を正とする。

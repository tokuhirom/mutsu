...
- **残件インベントリ（2026-06-29 実測、TAP 番号）**:
  ...
  - test 161: backtrace 4 frames（`.throw`/`die` を method/sub frame として
    backtrace に積む必要がある。test 157 で bare-block frame は実装済みだが、
    `X::AdHoc.new.throw` の `throw` frame と `die` の内部 frame は未モデル）

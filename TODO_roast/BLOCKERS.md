...
| ★achievable | `S02-types/array-shapes.t` | **42/43** (fails only test 31) | **43/43 with fudge** (raku's unfudged run aborts at the `#?rakudo skip` block line 167 — "partially dimensioned views NYI"; that block is skipped under fudge, so the file is a full oracle) | **Only T31 remains.** T43 (`my Int @a[2.5]` Rat shape, #4730) and T36-38 (`Z=` element-wise / shaped fill, this PR) are done. **T31** — `for @a.pairs -> $p { $p.value = X }` on a *multi-dimensional* shaped array: the pair key is an index tuple `(0,0)`, but the `.value=` writeback in `methods_mut_method_lvalue.rs` resolves the source element via `key.parse::<usize>()`, which only works for the 1-D (Int-key) case. Making shaped `.pairs` yield `ContainerRef`-valued pairs corrupts uninit leaves (`Any`→`Nil`) via `array_slot_ref` on the nested COW structure — needs the writeback path to handle tuple keys instead. Fixing T31 whitelists the file |

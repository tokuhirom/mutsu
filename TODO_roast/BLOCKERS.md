...
| `6.c/S02-names/pseudo-6c.t` | fails 52/161 (2026-07-17, was 56 before the `$::()` slice) | SORRY on ONE line | **The next slice here is `CALLERS::` (tests 119-127, the largest remaining cluster).** The `$::($name)::x` deref is DONE for the OUTER family (130/132/134/136) — the `$::(...)::y` spelling now shares one scope-chain resolver with the literal `$OUTER::y` (`compiler/lex_scope.rs`), so the two cannot drift. What is left splits into two roads, and they are **different mechanisms — do not conflate them**: (a) the `$::($x)::y` road (SymbolicDeref, done for OUTER); (b) the `::($x)::('$y')` road, which parses to `IndirectTypeLookup($x).WHO.{'$y'}` — a *stash* lookup, and where `::("OUR")`/`::("CORE")`/`::("GLOBAL")`/`::("PROCESS")`/`::("CALLER")` (50/51/53/61/79/84/116/125/139) actually live. `CALLERS::` fails in BOTH spellings (119 direct, 121 indirect), so it is a missing *feature*, not a deref gap; `Interpreter::get_caller_var` is the place. **It is NOT simply "OUTERS on the caller axis" — that guess is measurably wrong** (raku, 2026-07-17): the cascade only fires for `$*`-twigil names. `sub h1($f){$f()}; sub a2($f){my $v is dynamic = 7; h1($f)}; a2({ $CALLERS::v })` is **Nil**, and so is the `CALLERS::<$v>` spelling — a caller frame two levels out that declares an `is dynamic` lexical is NOT found. The same shape with `my $*v = 7` and `CALLERS::<$*v>` **is** found (7). One frame out, `CALLERS::<$u>` on an `is dynamic` lexical works (5). So for non-twigil names `CALLERS::` behaves as `CALLER::` does, and the roast tests are consistent with that (119 and 122 need no cascade; only 124 `CALLERS::<$*foo>` does, and it is a twigil). Establish the real rule against raku before implementing. Also failing: `::("UNIT")` (140 direct fails too), `::("SETTING")`, 137 `CALLER::OUTER::`. 137 `CALLER::OUTER::` is its own gap, measured: `Compiler::parse_caller_prefix` strips the `CALLER::` and hands `GetCallerVar` the *rest of the chain as a literal name* ("OUTER::x"), which is then looked up verbatim in the caller env — so any `CALLER::<pseudo>::` chain is Nil. Needs the caller frame to re-enter the pseudo-package dispatch, not a string lookup. **Separately, a real CALLER:: bug is sitting here, but it buys 0 roast tests** (no test in this file covers it — found by probing): mutsu's `$CALLER::v` reaches *past* the immediate caller — `sub h1($f){$f()}; sub a2($f){my $v is dynamic = 7; h1($f)}; a2({ $CALLER::v })` is Nil in raku, 7 in mutsu. Root cause measured: `push_caller_env` pushes `self.env.clone()`, a *flattened* env, so `get_caller_var` cannot tell a name the caller **declared** from one it merely **inherited** (h1 inherited `$v` from a2). Fixing it needs per-frame "what did this scope declare" data — the same class of lexical-vs-runtime question #4667 settled for `OUTER::`, but on the caller axis, where the answer is per-call-site and cannot simply be baked once. Compat-only, so weigh it against that cost. **"No oracle" is about the FILE, not the constructs**: raku SORRYs only on line 67 (`$MY::z ::= $y`, `::=` NYI), so individual snippets are verifiable in raku — do that first, this ledger's diagnoses have been wrong before (see news/2026-07.md). Two raku-verified facts worth keeping: a closure `{ $OUTER::c }` is itself a scope, so its OUTER is the block *enclosing the closure*, not the closure's caller; and an undeclared `$OUTER::x` is a `Failure` in raku but `Nil` in mutsu (open divergence, untested by this file) |
...
1. **`6.c/S02-names/pseudo-6c.t` — `CALLERS::`** (fails 52/161, measured). Still the most
   ...
   *measurement it produced* reshapes what is left: the remaining failures are not one gap but
   ...
   cluster) fails in the direct spelling too, so it is a missing feature, belonging in
   `Interpreter::get_caller_var` — but **do not start it from the "OUTERS on the caller axis"
   model, which raku disproves** (see the inventory row: the cascade fires only for `$*`-twigil
   names). The `::("OUR")`/`::("CORE")`/

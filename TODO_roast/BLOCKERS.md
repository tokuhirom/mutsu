...
- **優先度**: 最優先
- **難度**: Hard
- **現状**: 166/180 pass（180 run / plan 182、bad plan）。
- **理由**:
...
- **残件インベントリ（2026-06-29 実測、TAP 番号）**:
  ...
  - test 155: `X::TypeCheck::Binding`
  - test 157: backtrace の `.list` が 2 要素
  - test 161: backtrace 4 frames（正確な backtrace フレーム数）
  - test 166: `X::Method::NotFound` の message が空にならないこと
  - test 168: lexical `no worries` pragma が compiler warning を抑止
...
- **Next slice 候補**（いずれも単発・要設計）:
  test 166（message 空回避）や test 84（body 無し parametric-param sub）が比較的
  contained。test 16/179 の `X::Comp::Group` 構造化は再利用性が高いが重い。
- **Canary tests**:

...
- **優先度**: 最優先
- **難度**: Hard
- **現状**: `TODO_roast/S32.md` ベースで 160/180 pass まで前進済み
- **理由**:
...
- **Next slice**:
  compile-sorrow accumulator は generalize 済み（regex `** N..M` malformed range と
  null operator `infix:<>` の 2 ケースで `X::Comp::Group` に panic/sorrows/worries を
  畳む土台を整備、tests 20 + 26 通過）。次は **source-format preserving warning**
  （slice 2）— `Expr::Literal` に source text/span を持たせて sink warning と
  `X::Syntax::Confused` の `pre`/`post` に通す。これは misc.t の test 17 や
  sink-format クラスタ（121/139-142）を広くまとめて解く。
- **Canary tests**:

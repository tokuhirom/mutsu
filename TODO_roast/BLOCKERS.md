...
- **残件インベントリ（2026-06-30 実測、TAP 番号）**:
  - test 16: `X::Comp::Group` — `CATCH { when X::Y {} }` で undeclared bareword
    ...
    sorrow=X::Syntax::BlockGobbled）。`when` の condition が宣言済み型かどうかを
    parse 時に判別する必要があり、`when DeclaredType {}` を壊さずに実装するのが
    難しい（残件）。
...
- **Next slice 候補**（いずれも単発・要設計）:
  test 16 の `when`/CATCH gobble 検出（X::Comp::Group）— 宣言済み型判別が必要で
  `when DeclaredType {}` を壊さない実装が課題。
- **Canary tests**:

...
| `integration/advent2013-day18.t` | 9/10 | PASS ★ | The rule-embedded dynamic-var declaration (`rule deal { :my %*PLAYED = (); … }`), sigspace `[ <card> ]**5`, `<?{ … $/.lc … }>` matched-so-far `$/`, and action-method + code-assertion sharing of `%*PLAYED` all work now (0/10 → 9/10; pin t/grammar-dynamic-var-decl.t). The **only** remaining failure is test 10: a `<?{ … }>` code assertion's `@dups.push(…)` side effect (an `our @dups` **array**) does not persist out of the `&self` backtracking matcher's scratch interpreter — a hash element write (`%*PLAYED{$c}++`) persists via in-place `arc_contents_mut`, but `Array.push` COWs the shared Arc. Needs code-assertion outer-variable writeback (distinct architectural gap) |

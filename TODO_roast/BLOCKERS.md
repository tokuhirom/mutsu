...
| **④ エラーメッセージ品質** | 2 | `error-reporting.t`（**26/33**・元 4/33）・`weird-errors.t`（**30/36**・元 26/36） | 「Parse error contains line number」等、**例外の文面・行番号・バックトレース**を検査するテスト。#4539 で全ランタイムエラーに backtrace・is_run=CLI 同一 stderr・Backtrace.new/full 等を実装。error-reporting 残 7 = コンパイル時 undeclared routine 検出（test 5/23・最大の的）・モジュール file の backtrace frame（15）・Failure dual backtrace（20）・map 内 return（21）・`%::{''}`（25）・type capture 継承（30）。weird-errors 残 = `say(;:[])`（11）・`::a`→X::NoSuchSymbol は BareWord に `::` 情報が要る（20）・our method invocant 型（29）・`new Foo:`（32）・`hash a=>1`（33） |

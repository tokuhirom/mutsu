...
- [ ] roast/S12-introspection/walk.t
  - Deferred (in too_difficult.txt). Subtest 1 "Basics": ~12 failures because WALK currently returns a precomputed Sub instead of a WalkList of per-method candidate callables; cand_order(@cands, $x) treats @cands as a single sub and joins all results into one space-separated string. Need a real WalkList type with `.invoke`/`.reverse`/`.quiet` methods, postfix `()` invocation, lazy iteration, and support for `:canonical`/`:super`/`:breadth`/`:descendant`/`:ascendant`/`:preorder`/`:omit`/`:include`/`:roles` flags producing distinct traversal orders. Subtest 3 "Lazyness" requires lazy method dispatch tied to consumer iteration. Quiet mode must wrap thrown method errors in Failure with `.exception`. Slip returns must be preserved. Spans runtime/methods_walk.rs, runtime value types, exception/Failure handling, and MRO traversal — too broad for a focused fix.

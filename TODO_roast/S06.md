...
- [x] roast/S06-signature/slurpy-params.t (DONE, PR #3918 — 86/86 whitelisted)
  ...
  - Lazy infinite slurpy (`oneargraw(1..*)[^5]`) and Seq/List identity (`+foo`
    keeps a single Seq as Seq while `+@foo` converts it to List) now both work,
    plus Seq single-pass consumption (re-iterating a `+foo` Seq throws
    `X::Seq::Consumed`). Fixed in PR #3918.

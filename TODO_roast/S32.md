...
- [x] roast/S32-io/socket-recv-vs-read.t
  - Whitelisted. Test 11 (`read blocks until it has enough data`) needed the `.tap`
    callback — driven asynchronously on the socket-listener worker thread — to observe
    the parent thread's post-registration reassignment of the captured `$send-rest`
    Promise. Fixed by marking closure args to supply-consuming methods (`tap`/`act`)
    as escaping, so the captured-and-mutated lexical is boxed into a shared
    `ContainerRef` cell (same escape rule `start {...}` uses).

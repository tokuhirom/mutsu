...
- [ ] roast/S32-exceptions/misc.t
  - 156/180 pass (plan 182; 24 fail, 2 not-run). This is a very broad
  ...
  - **2026-06-29: test 15 done (X::Comp::Group, gobbled block)** — a brace block
    ...
    - test 20 `/m ** 1..-1/` — needs the regex parser to ACCUMULATE multiple
      sorrows (MalformedRange + UnrecognizedMetachar) plus a panic; mutsu throws
      the first regex error eagerly. Needs a regex-parser sorrow accumulator.
    - test 26 `sub infix:<> (){}` — panic X::Syntax::Extension::Null needs `pre`/
      `post` source spans + a `worries[0].payload` ("Pair with <> really means an
      empty list"); the pre/post is the same source-span-threading blocker as the
      sink-format cluster.

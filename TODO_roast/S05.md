...
- [x] roast/S05-capture/array-alias.t
  - 50/50 pass. Implemented `@<name>=(...)` array-sigil capture aliasing in the
    regex parser (recognized in both Match and Validate modes, mirroring the
    `$<name>=` scalar alias) with a `force_list_capture` flag on `RegexToken`
    that marks the named capture as list-context (`named_quantified`), so even a
    single non-quantified `@<foo>=(.(.))` yields a one-element List. Also fixed a
    sigil-alias-on-capturing-group bug: `$<x>=(\w)+` / `@<x>=(.(.))+` must NOT be
    wrapped as a whole-span single Match (the `wrap_named_quant` path); a
    capturing group aliases each iteration's Match into a List. Finally added a
    `Match` arm to `dispatch_at_key_method` so hyper named-capture access
    (`@<chars>».<spaces>`, which dispatches `.AT-KEY` per element) resolves named
    subcaptures. (`@var=(...)` lexical/package array captures remain `#?rakudo
    skip` in this file, so they don't block.)
...
- [ ] roast/S05-capture/hash.t
  - Unpassable. Hash captures inside regexes `%<name>=(...)` are removed/reserved in current Raku and produce a compile error. raku itself also rejects it with `The use of hash variables in regexes is reserved` (an obsolete feature dating back to 2004 Perl6::Rules). mutsu likewise rejects it, matching raku's behavior. No implementation needed.
...
- [x] roast/S05-match/capturing-contexts.t
  - 64/64 pass (whitelisted). Test 64 (capture markers in grammar tokens) now passes via two
    ...
    Tests 53-54 (`( 'rule1' || 'rule2' )* %% (.+?)`) now pass: the `%%` separator
    path backtracks the frugal separator (and an optional trailing separator)
    against the outer `$` anchor. `match_separated_quantifier` now enumerates
    every `atom (sep atom)*` chain via `enumerate_separated_chains` (DFS that
    tries each separator length in priority order so a longer ` foo ` admits the
    next `rule2` atom) and emits per-chain trailing-separator candidates, so
    `g0=rule1,rule2` / `g1=" foo "," bar"`. The parser-level LTM-expansion guard
    (`regex_parse.rs`) that used to route `||`/frugal-separator patterns to the
    capture-renumbering string expansion now defers to this native path whenever
    a capture is present (`t/separator-backtracking.t`).

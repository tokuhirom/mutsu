...
## 2026-07-15: J4d 第4〜6弾 完了 — ゲート再判定・ADR-0004 クローズ（#4534/#4537/#4540）

J4d の残りを profile 駆動で 3 スライス実施し、**ADR-0004 のフェーズ計画を完遂・クローズ**した。

- **第4弾（#4534）: dispatch テーブルの FxHash 化 + 引数スキャン融合**。fib(28) after-profile の
  ~11% が SipHash（Symbol キーの light-call キャッシュ + String キーの compiled_fns 表 —
  毎呼び出し probe）だった。`CompiledFns` 型 alias（FxHashMap・84 箇所）+ Interpreter の
  dispatch キャッシュ 5 つを FxHashMap 化 + pos-light hit path の junction/callsite-marker
  スキャンを 1 パス化（nanbox decode 1 回/arg）。fib(28) 0.19-0.20 → 0.17s（-12%）。
- **第5弾（#4537）: light-call の caller-env フレーム再利用（CoW write latch 方式）**。
  第4弾後の profile は `positional_light` self の ~26% が env 儀式（clone → `scoped_child`
  → swap → drop）の Arc refcount `lock inc/dec` 対だった。caller の overlay が
  プロセス共有 empty singleton のままなら（`overlay_is_shared_empty`）、fresh overlay の
  設置は観測上恒等なので env をそのまま使う。健全性は静的解析でなく**動的 latch**:
  singleton が write 検出器を兼ね（初回 by-name write が `cow_mut` の CoW で un-share）、
  unwind が latch 発火を見て scoped-overlay の return merge を in-place 再現
  （`retain_overlay`）。第3弾で却下した静的 overlay-skip と違い flaky になり得ない。
  ＋GC call-safepoint の `PENDING.swap`（locked RMW/call）を test-and-test-and-set 化。
  fib(28) 0.16-0.17 → 0.13-0.14s（-19%）。
- **第6弾（#4540）: readonly-set の Arc スナップショット → mutation journal**。フレームごとの
  `readonly_vars` snapshot（Arc clone+drop = `positional_light` self の ~15%）と、light
  method frame 用の手書き delta リスト（`readonly_added`/`readonly_removed`）の**二重機構を
  journal 1 本に統合**。mark/unmark が実変更だけを記録し、`exit_readonly_frame` が逆再生。
  Scope sentinel で相殺 peephole を同一フレーム内に限定（外側フレームのエントリを食うと
  `is copy` param が caller の readonly を壊す — t/copy-param-shadows-caller-readonly.t が
  開発中に捕捉）。トップレベルでは journaling off ＝生涯増殖なし。fib(28) cycles
  ~526M → ~494M（-6%）。

**bench CI 実測**（ratio = 対 raku・正本）: fib+jit **0.34 → 0.28**（`d6d405cc` → `c397b90b`）・
bench-fib+jit **0.66 → 0.56**。local 累計 = fib(28) pre-J4d ~0.4s → **0.13s**。

**J4「int ループ 5-10x」ゲートの再判定**（ADR-0004 2026-07-15 追記）: int-arith の on/off 比は
1.24x で文言未達のままだが、この期待値は J4c/J4d 以前のインタプリタが分母。共通コスト
（dispatch・call 儀式）の削減は両系列を同時に速くするため比率では原理的に届かず、絶対性能では
int-arith+jit = raku の **0.18x（≈5.5 倍高速）**。「tight numeric loop の native 級実行」は
絶対値で実現済みと判定し、比率ゲートは obsolete としてクローズ。

**設計評価の記録**: PLAN 旧①「CallFunc opcode への per-call-site inline cache」は不採用
（FxHash 化後の二重 probe は ~10-15ns/call・残り利得 ~3% に対し Arc 化+map-id 機構が過大）。
旧②「JIT→JIT 直接呼び出し」も専用経路は作らず、interpreter/JIT 共有の light-call 経路の
固定費削減（第5・6弾）で同等の利得を回収 — profile 上、呼び出し儀式が dispatch probe の
3 倍超だった。残る on/off 共通固定費（SetLocal env-mirror 等）の根治は §6 lexical-slot
campaign の領域。
## 2026-07-15: BLOCKERS クラスタ⑤（個別機能ギャップ）を 8 本 whitelist — 残り 2 本

`integration/` の個別機能ギャップ 10 本のうち 8 本を 1 日で whitelist（#4533 / #4535 / #4536 /
#4538 / #4541 / #4542）。各修正は単発 fix ではなく一般機構の改善:

- **`advent2010-day22.t` / `advent2009-day20.t` / `advent2009-day18.t`（#4533）** —
  builtin 型の `.^attributes`（`Rat` の `$!numerator`/`$!denominator` 等のモデル表）と
  非空 `.^methods(:local)`（宣言済み own-method リスト）; `signature.raku` のリテラル
  default 表示（`$blah = 5`）・sigil 由来の `Positional`/`Associative`/`Callable` param 型
  （`Int @x` は `Positional[Int]`）・stub `{...}` body の map callback を強制まで遅延;
  `my Cup of EggNog $mug` の `.WHAT`/`.raku` が型引数を保持（`SetVarType` が
  ParametricRole を実体化、`nominal_type_object_name_for_constraint` が `[` で切らない、
  ParametricRole の raku/gist repr 追加）。
- **`advent2010-day14.t`（#4535）** — 実体は `nextsame` ではなく: `method m { say "y" }`
  のような I/O-only メソッドが `Say`/`Put`/`Print`/`Note` opcode の `has_calls` 漏れで
  `can_skip_merge=true` と誤分類され、メソッド exit が stale env スナップショットを復元して
  user `$*OUT` override の captured-outer 書き込みを破棄していた。4 op を `has_calls` に追加。
- **`advent2013-day10.t`（#4536）** — 演算子 adverb を「式ルートの演算子」に付与する
  `attach_trailing_adverbs`（`1+2-3 :adv` は最外の `-`、右結合 `1**2**3 :adv` は最左 `**`）;
  user prefix の operand 内で postfix の space-form colonpair grab を保留し prefix 呼び出しに付与
  （括弧はコンテキストをリセット）; `- * / **` の user infix override を `+` と同じく VM で
  ディスパッチ（従来は silent に native 実行）; 演算子呼び出しの Empty オペランド保持;
  `m:nth[5]`（角括弧 adverb 引数）を X::Comp::Group で拒否。
- **`advent2011-day04.t`（#4538）** — wrap 済み sub の**再帰呼び出しが wrap chain を再入**
  （`is Cached` メモ化イディオムが全再帰レベルをキャッシュ）。broad な `is_wrap_dispatching`
  ガードを callsame/callwith 専用の one-shot `wrap_skip_once` に置換し、`nextcallee` の返す
  wrappee は `__mutsu_wrap_direct` マーカーで常に直接実行（CI で発覚した defer-call.t の
  無限再帰を fix-forward）。
- **`advent2011-day10.t`（#4541）** — `--doc` が pod ブロックを宣言子ブロックより先に
  ソース順で描画・`=item` を `  * ` で描画; `use Pod::To::Text` を builtin module 認識
  （JSON::Fast 方式）し、Pod オブジェクト木を実際に描画する native `pod2text` を追加。
- **`advent2009-day24.t`（#4542）** — 派生 grammar が proto 候補を追加すると**継承候補が
  全滅**していた（MRO walk が最初のヒット scope で停止）。token def/pattern 解決の 3 walk を
  「全クラスから候補をマージ・candidate identity（`name:sym<X>`）で最派生優先 dedupe」に統一。

残りは `advent2011-day07.t`（`Metamodel::GrammarHOW` 継承 = メタモデル大工事）と
`precompiled.t`（precomp 基盤）の 2 本。

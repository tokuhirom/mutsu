...
async IO / Proc::Async / socket supplies を残すのみ。ただし test 53（`supply {}` の whenever
相互排他順序）は full-file 並列負荷下でのみ稀に race する既知フレーキーで、単体では安定して通る
（CLAUDE.md「Known flaky tests」の S17-* concurrency に該当）。

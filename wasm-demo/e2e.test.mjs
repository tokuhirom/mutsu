...
/** Run the playground's editor buffer and return the output pane's text. */
...
try {
  ...
  /* =============================================================== *
   * Playground — write a program, run it, read the output
   * =============================================================== */

  console.log('Test: playground WASM initialization');
  await page.goto(`${BASE}/playground.html?lang=en`, { waitUntil: 'networkidle' });
  await page.waitForFunction(() => document.body.dataset.ready === '1', { timeout: 30000 });
  assert(await page.locator('#repl-input').count() === 0,
         'the playground has no REPL prompt — that is a page of its own');
  assert(await page.locator('#repl-cta').count() === 1,
         'and it links to the REPL for people who want a session');

  // --- Test: running the buffer shows what it printed ---
  console.log('Test: playground run');
  assert(await runEditor(page, 'say "hi";') === 'hi', 'Run shows what the program printed');
  assert(await runEditor(page, 'my $x = 1;') === '(no output)',
         'a program that prints nothing says so');

  // --- Test: every run starts from a clean interpreter ---
  console.log('Test: playground runs are isolated');
  ...
  const fbLines = (await page.textContent('#output')).trim().split('\n');
  ...
  // --- Test: Reset code restores the loaded example ---
  console.log('Test: Reset code');
  await page.fill('#code', 'say "mine";');
  await page.click('#reset-btn');
  assert((await page.inputValue('#code')).startsWith('for 1..20'),
         'Reset restores the example that was loaded');
  ...
  await page.waitForFunction(() => !document.getElementById('run-btn').disabled, { timeout: 30000 });
  assert((await page.textContent('#output')).trim() === '42', 'Ctrl+Enter runs the editor buffer');

  // --- Test: Error handling ---
  console.log('Test: playground error handling');
  const pgErr = await runEditor(page, 'die "oops";');
  assert(pgErr.includes('Error'), `die produces an error message (got: ${JSON.stringify(pgErr)})`);
  assert(await page.locator('#output.error').count() === 1,
  ...
  console.log('Test: REPL WASM initialization');
  await page.goto(`${BASE}/repl.html?lang=en`, { waitUntil: 'networkidle' });
  await page.waitForFunction(() => document.body.dataset.ready === '1', { timeout: 30000 });
  assert(!(await page.isDisabled('#repl-input')), 'the prompt is enabled once WASM is ready');
  assert(await page.locator('#code').count() === 0,
         'the REPL page has no editor — the playground is where programs are written');

  // --- Test: an expression shows its value ---
  ...
  // --- Test: state persists across lines (the whole point of the page) ---
  ...
  await replEval(page, 'class Point { has $.x; method twice { $.x * 2 } }');
  assert(await replEval(page, 'Point.new(x => 21).twice') === '42',
         'a class declared earlier can be instantiated');
  ...
  assert(await page.textContent('#repl-prompt') === '*',
         'the prompt switches to the continuation marker');
  ...
  await page.press('#repl-input', 'ArrowDown');
  assert(await page.inputValue('#repl-input') === '', 'ArrowDown returns to the fresh line');

  // --- Test: the "Try" chips load a line without running it ---
  console.log('Test: try chips');
  const chip = page.locator('.chip-code').first();
  const chipText = (await chip.textContent()).trim();
  await chip.click();
  assert(await page.inputValue('#repl-input') === chipText,
         'clicking a suggestion puts it in the prompt, ready to be read');
  await page.fill('#repl-input', '');

  // --- Test: Error handling ---
  console.log('Test: REPL error handling');
  ...
  // --- Test: Clear screen keeps the session ---
  console.log('Test: Clear screen');
  await page.click('#clear-btn');
  assert(await page.locator('#repl-log > div').count() === 0, 'Clear empties the transcript');
  assert(await replEval(page, '$x') === '40', 'but the session still knows what was declared');

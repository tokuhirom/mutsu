...
# closures over the same lexical share one cell (loop-body AND non-loop), while
# loop-body `my` stays per-iteration fresh.
...
# Phase 1 / lever C: boxing now extends to non-loop captured-mutated `$` scalars
# (the GetLocal/GetGlobal into_deref chokepoint decont's the boxed cell on read). ---
{
    sub make-pair {
        my $v = 1;
        my $g = { $v };

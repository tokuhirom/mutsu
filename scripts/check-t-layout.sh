...
# Declared subcategories, as '<category>/<subcategory>'. A category nests one
# level once it passes ~200 files (§2). Generated by scripts/migrate-t-layout.py,
# whose SUBRULES table is the source of truth; keep the two in step.
SUBCATEGORIES="
collections/array
collections/hash
collections/lazy-seq
collections/range-pair
collections/set-bag-mix
